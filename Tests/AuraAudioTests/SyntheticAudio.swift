...
enum SyntheticAudio {
    ...
    static func toneBurst(
        ...
        for frameIndex in 0..<totalFrames {
            ...
            let isBurst = frameIndex >= leadingSilenceFrames &&
                frameIndex < leadingSilenceFrames + burstFrames
            if isBurst {
    ...
    static func intermittentTone(
        ...
        for frameIndex in 0..<totalFrames {
            ...
            let amplitude: Float = isActive ? baseAmplitude * Float(0.7 + 0.3 * generator.nextDouble()) : 0
            var samples = [Float](repeating: 0, count: frameLength)
            let globalOffset = frameIndex * frameLength
            for sampleIndex in 0..<frameLength {
                let t = Double(globalOffset + sampleIndex)
