...
struct AuraSTTEngineTests {
    ...
    func partialThenStable() async throws {
        let engine = DeterministicMockSTTEngine(
            script: [
                DeterministicMockSTTEngine.MockSegment(text: "hello", expectedFrameCount: 6, alternatives: ["hullo","allo"]),
            ],
    ...
    func cancellationDoesNotLeakResults() async throws {
        ...
        #expect(!resultsBox.results.contains { $0.isStable }, "Cancellation must prevent stable segment emission")
    }
