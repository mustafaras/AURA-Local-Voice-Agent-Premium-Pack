...
- **Mitigations:** every `XConfiguration.validate()` runs before use and rejects structurally invalid input; `PolicyConfiguration.validate()` rejects an `allowByDefaultTiers`/`denyByDefaultTiers` overlap. Phase 24 adds registry-owned types and directional security constraints: project/session values cannot raise allow-by-default risk, lower mandatory confirmation, widen network access, enable raw telemetry, or exceed a machine-policy resource bound.
- **Detection:** Phase 24's `AuraConfig.ConfigurationEngine` resolves only registry-known keys, records unknown-key warnings, provides a default diff and source layer for every effective value, and appends a bounded durable audit record for accepted or rejected patches.
- **Recovery:** Every accepted mutation first retains a compatibility snapshot. Rollback is one atomic persisted state-envelope replacement and survives restart; ephemeral session overrides are cleared during restart.
- **Residual risk:** The project-layer weakening gap is closed for every key registered in the Phase 24 schema. Future configuration keys can reintroduce risk if their registry definition incorrectly marks a security value unrestricted; ADR-032 therefore makes security classification and adversarial non-weakening tests mandatory for each new security-relevant key. Direct local database tampering remains outside the repository-project threat boundary and requires local filesystem access.
- **Test evidence:** `Tests/AuraConfigTests/ConfigurationEngineTests.swift` covers project attempts to raise allow-by-default risk, machine-bound relaxation, allowed hardening, atomic write failure, override revocation, restart rollback, and session expiry.
