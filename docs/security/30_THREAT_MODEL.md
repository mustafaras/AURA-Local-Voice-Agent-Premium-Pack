...
- **Mitigations:** `PluginVerifier.verify` checks, in order, structural validity → content-hash integrity (real SHA-256 over the actual bundle payload) → vendor trust (deny-by-default `PluginTrustRegistry`, empty unless explicitly configured) → Ed25519 signature validity over a canonical, order-independent `signedPayload`; every capability is mapped to a scoped `Grant` only after `.verified`, and every lifecycle transition (install/enable/disable/quarantine/uninstall) is itself a `PolicyEngine.evaluate` call — a plugin cannot install or re-enable itself by simply being asked to.
- **Detection:** `PluginVerificationEvent` records every verification attempt and its outcome, success or failure, for later audit.
- **Recovery:** `quarantine` is a one-way safety valve (no `unquarantine`); `uninstall` revokes every grant the plugin was issued and retains the record for audit, per `docs/subsystems/23_PLUGIN_SYSTEM.md`'s "disable, quarantine, and uninstall workflows."
- **Residual risk:** there is no real plugin execution runtime yet — no sandboxed XPC/helper process exists (deliberately deferred to Phase 23, "Verified Plugin and Adapter Marketplace"), so `PluginRegistry` currently governs *bookkeeping and authority*, not actual code execution; there is also no real vendor PKI/distribution mechanism — `PluginTrustRegistry` is a local, operator-configured trust list, not a connection to any external directory.
...
- No real plugin execution runtime exists yet (entry 11) — Phase 23 scope.
