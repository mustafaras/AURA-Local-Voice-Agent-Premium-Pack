...
# SP-010 — Provider, Account, and UI Composition

## Mission

Wire read-first browser, mail, calendar, and contacts adapters through provider/account onboarding, policy, dialogue, and UI.
...
- `R5 prompt`
- `ADR-040`
- `Gmail/EventKit/Contacts adapters`
- `AuraKernel`
- `DialogueEngine`
- `R9 capability/permission UI`
...
1. Implement explicitly authorized test-account/profile onboarding with incremental scopes and Keychain references.
2. Connect provider transport, account ambiguity, offline/degraded state, revocation, and truthful capability health.
3. Expose the four read capabilities through registry, dialogue, and UI without enabling send/mutation.
4. Add redaction and secret-leakage tests across prompts, logs, events, and errors.
...
- Why is SP-011 now safe to start?

## Required records

- Evidence ID prefix: `EV-SP-010`; include timestamp, branch/commit, command or procedure, environment, result, artifact path/hash, scope, and limitations.
...
Each read-first capability has a real composition path, account isolation, scope boundary, and actionable UI state.

## Stop condition

If any required evidence, authority, cognitive answer, postcondition, or validator result is missing, keep SP-010 `in_progress` or `blocked`, record the exact blocker, and do not proceed to SP-011.
