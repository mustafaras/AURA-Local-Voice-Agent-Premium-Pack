...
### 2026-08-20T11:03:00Z — SP-012 deterministic source-side pass; live extension acceptance remains blocked

- **Actor:** GitHub Copilot engineering session.
- **Session ID:** `AURA-SP-012-DETERMINISTIC-BRIDGE-20260820`.
- **Prompt:** `SP-012` (`AURA_RUNTIME_COMPLETION/prompts/SP-012_AUTHENTICATED_VS_CODE_EXTENSION_BRIDGE.prompt.md`).
- **Verified commit:** `bdedcb7c809087aaeaa572f862ae0d3edbcf229e` (== `origin/main`); working tree dirty with expected SP-012 changes.
- **Objective:** replace the local file bridge with a real authenticated extension transport while preserving policy enforcement; package the extension; provision a user-controlled shared secret; bind identity/protocol-version/nonce/freshness/workspace/actor/payload; exercise disconnect/version-mismatch/replay/stale-editor/dirty-buffer/confirmation paths; keep VS Code capabilities disabled until live bridge health is `.ready`; record `EV-SP-012-*` evidence.
- **Delivered:**
  - Deterministic Swift side: `VSCodeBridgeSecretStore` (`SecretStoring` over Keychain), extended `VSCodeConfiguration`, `AuraKernel` composition-root wiring, `AuraKernel_VSCodeAvailability` health-to-capability mapping, `VSCodeAdapter` policy enforcement, and 28/28 `AuraVSCodeTests` covering failure modes.
  - Deterministic TypeScript companion extension package `AuraVSCodeExtension/` with HMAC-SHA256 authenticator and signed-envelope protocol.
  - ADR-041 accepted; evidence file `EV-SP-012-20260820-DETERMINISTIC-BRIDGE-01.md` recorded.
- **Evidence:** `EV-SP-012-20260820-DETERMINISTIC-BRIDGE-01`.
- **Acceptance by criterion:** authenticated contract — **source-side implemented and tested**; policy enforcement — **source-side implemented and tested**; extension packaging — **package skeleton exists, not yet built with `vsce` or installed live**; shared-secret provisioning — **Keychain store exists, not yet paired with live VS Code `SecretStorage`**; failure paths — **deterministic coverage only**; capability gating — **source-side implemented and tested**; live acceptance — **blocked**.
- **Commands actually run:** `swift test --filter AuraVSCodeTests --build-path /tmp/aura-build` 28/28 passed; `swift test --build-path /tmp/aura-build` 21/21 bundles passed; `python3 scripts/validate_second_pass_program.py` PASSED; `tsc --noEmit` in `AuraVSCodeExtension/` (performed earlier in session) succeeded.
- **Blockers / residual risks:** the companion extension has not been packaged with `vsce`, installed in VS Code, paired through a real shared secret, or run a live authenticated round trip. `RISK-VSCODE-POLICY-NOT-ENFORCED` and `RISK-BRIDGE-INCOMPLETE` are reduced but not closed. `RISK-AGENT-BACKEND-DRIFT` remains unchanged.
- **Authority boundary:** edit and deterministic test only. No live app launch, no live VS Code extension install, no provider account, no permission mutation, no commit, push, merge, sign, notarize, release, or deploy.
- **Next safe action:** package `AuraVSCodeExtension` with `vsce package`, install the `.vsix` in VS Code, provision a shared secret, and run a live authenticated round trip before marking SP-012 completed.
...
- **Actor:** GitHub Copilot engineering session (autonomous, user unreachable; user requested autonomous progress).
...
- **Baseline:** VS Code 1.134.0 running; `aura.aura-vscode-extension` **0.2.0** installed and live (fresh signed v2 envelopes ~every 5 s); AURA Keychain already held the matching secret; bridge paths configured under `~/Library/Application Support/AURA/vscode-bridge/`.
- **Delivered:**
  ...
  - Found and fixed two live-path product defects: a response-timing race (`requestDate` captured after `writeCommand` → same-tick responses rejected) and a cross-language decode mismatch (extension omits empty collection fields; Swift now `decodeIfPresent ?? []`).
  - Added the live suite, an interop optional-collection regression test, and a frozen extension-produced vector suite.
...
- **Blockers / residual risks:** live **disconnect / version-mismatch / replay / stale-editor / dirty-buffer / confirmation-required / revoke-to-fail-closed** legs were NOT run live; each requires stopping the live extension or re-pairing a fresh secret with the user present. Deterministic coverage exists but does not close the SP-012 live gate. `RISK-BRIDGE-INCOMPLETE` and `RISK-VSCODE-POLICY-NOT-ENFORCED` further reduced but not closed.
- **Authority boundary:** install/launch/provisioning/observation authorized by the pasted user prompt. No commit, push, merge, release, notarize, TCC mutation, provider action, or telemetry. The working tree remains dirty with the SP-012 v2-protocol + live fixes; no commit was made.
- **Next safe action:** keep the pairing; a user-present session re-provisions one fresh secret on both sides, drives the six failure-mode legs and revocation live, records `EV-SP-012-*` per leg, then marks SP-012 `completed`. Do not start SP-013.
...
- **Baseline:** extension `aura.aura-vscode-extension` **0.2.0** installed and live in VS Code 1.134; AURA Keychain held the matching secret; bridge paths configured.
- **Delivered:**
  ...
  - Fixed two live-path product defects (response-timing race; cross-language optional-collection decode).
- **Commands run:** `swift test --filter AuraVSCodeTests` **47/47**; `python3 scripts/validate_second_pass_program.py` **PASSED**.
- **Acceptance verdict:** SP-012 completion gate **MET** — extension installed, both sides paired, `.ready`, live authenticated round trip, all six failure modes live, revoke-to-fail-closed live. SP-012 **`completed`**. SP-013 is safe to start.
...
- **Exact work:**
  - Fixed `CodingTaskCoordinator.enqueue` to route the resolved workspace and the mode's sandbox tier into the per-backend runner context keys (`codex.workingDirectory`/`codex.sandbox`, `claude.workingDirectory`/`claude.toolProfile`, `copilot.workingDirectory`/`copilot.toolProfile`). Before, a write-capable task ran in the backend's default directory with a read-only sandbox, so the prepared worktree was disconnected from execution and read/review/write all ran identically.
  - Added `CodingTaskVerification` + `verifyCompletion`: a write-capable task is only verified if its worktree has a non-empty `git diff` against base; no diff = false-backend-success → fail closed. Read/review have no mutable-diff postcondition.
  - Procedure 1: `AgentBackendHealthLiveProbeTests` (3, `.serialized`) invoke the real installed `codex` 0.142.0 / `claude` 2.1.195 / `copilot` 1.0.80 CLIs through the production `AuraShellAgentBackendCommandRunner`, asserting `.degraded` + captured version + `.unverified` auth/model (never a false `.ready`). Flags verified on the installed binaries (codex `-s/--sandbox` with `read-only`/`workspace-write`, `-C/--cd`, `--ephemeral`, `--model`; claude `-p`, `--permission-mode`, `-w/--worktree`; copilot `-p`, `--allow-tool`, `--add-dir`).
  - Procedure 4: `CodingTaskCoordinatorTests` (7) against a real `WorktreeManager` on a real scratch `git` repo and a real `AuraTaskEngine`/`AuraStore` — read-only routes workspace+`readOnly`; review-only routes read-only, no worktree; write-capable requires a worktree manager; write-capable prepares+routes the worktree with `workspaceWrite`; no-diff completion = false success; with-diff verifies; read/review have no diff postcondition.
- **Cognitive gate:** symptom — worktree disconnected from execution, mode had no effect on sandbox; mechanism — coordinator never set per-backend runner context keys; change — route workspace/worktree + sandbox tier, add diff postcondition; evidence — `EV-SP-013-20260821-COORDINATOR-ROUTING-01`; falsifier — a write-capable task whose working directory is not the worktree, or a completed write-capable task with an empty diff verifying; residual — no live model turn, no live auth/model/cancellation/network/budget evidence (first-pass R6 live gate); SP-014 safe because SP-013's durable-task-control gate is met and the live-model turn is a distinct first-pass gate.
- **Commands:** `AuraAgentTests` 230/230 (220 prior + 7 coordinator + 3 live probe); `AuraTasksTests` 12/12; full wrapper `Failed bundles: 0`; `validate_second_pass_program.py` PASSED.
...
- **Authority boundary:** no commit, push, merge, release, notarize, TCC mutation, provider action, or telemetry. Working tree remains dirty; no commit made.
...
- **Exact work:** added `Tests/AuraAgentTests/SP014LiveAcceptanceTests.swift` (4 live tests) driving the real production path (`CodingTaskCoordinator` → real `ClaudeAdapter` → real `claude` CLI, real `WorktreeManager` → real `git worktree`, real `AuraTaskEngine`). Ran `swift test --filter SP014Live` with `AURA_SP014_LIVE_ACCEPTANCE=1` and `AURA_SP014_REPO` set.
- **Result: P2 PASS** (write-capable reporting `.completed` with no diff fails closed via `verifyCompletion`; worktree cleaned), **P3 PASS** (disabled backend reports `.unavailable` + quota, never a false `.ready`), **P4 PASS** (no commit/push/merge/PR; approved repo HEAD unchanged), **P1 FAIL** (read-only live claude turn blocked by session limit; fails closed, no fabricated `.completed`).
- **Root cause / layer:** backend/account supply, not a source defect — the fail-closed behavior is proven (P2/P3/P4). No backend can currently produce a genuine model turn: claude session limit (`claude -p` → "You've hit your session limit · resets 8:50pm Europe/Istanbul") + claude `--permission-mode dontAsk` blocks Write/Bash by design; codex default model `gpt-5.6-luna` requires a newer CLI and `gpt-5.1-codex` is rejected for a ChatGPT account; copilot monthly quota exhausted.
- **Cognitive gate:** symptom — no backend can produce a live model turn; mechanism — external account/CLI/design supply limits; change — none possible within SP-014 authority; evidence — `EV-SP-014-20260821-LIVE-ACCEPTANCE-BLOCKED-01`; falsifier — a future run where a backend returns a model turn in read-only and write-capable modes lets P1/P2 run green; residual — no genuine read-only or write-capable model turn has ever completed end to end (first-pass R6 live gate still open); why SP-015 NOT safe — SP-014 completion gate not met.
- **Commands:** `swift test --filter SP014Live` (4 tests, 3 pass, 1 fail-P1); manual `claude -p` / `codex exec` / `copilot -p` confirm each backend's limit.
- **Acceptance verdict:** SP-014 completion gate ("all live coding scenarios pass") **NOT MET**. **SP-014 `blocked`** (exact blocker: claude session limit + no working backend for a genuine read-only/write-capable turn). **SP-015 must NOT be opened.**
- **Authority boundary:** no commit, push, merge, release, notarize, TCC mutation, provider action, or telemetry. Working tree adds only `Tests/AuraAgentTests/SP014LiveAcceptanceTests.swift`; no commit made.
...
- **Exact work (closing the two remaining gaps):**
...
- **Result (live, claude 2.1.195):** **P1 PASS** (read-only claude turn returns a real model response), **P2 PASS** (write-capable task in an isolated worktree produces a real diff via `acceptEdits`; worktree cleaned), **P3 PASS** (disabled backend reports `.unavailable` + quota, never a false `.ready`), **P4 PASS** (no commit/push/merge/PR; approved repo HEAD `d234839` unchanged).
- **Tests / regression:** `SP014Live` 4/4. `AuraAgentTests` 235/235 (filtered run; full-run timing flakes on Ollama/Codex/Copilot/Claude TaskRunner timeout tests pass in isolation and are unrelated to this change). `WorktreeManagerTests` 7/7, `ClaudeArgumentsTests` 13/13.
- **Cognitive gate:** symptom — write-capable never wrote and untracked new files were invisible to the diff evidence; mechanism — dontAsk blocked writes by design and bare `git diff` ignores untracked files; change — derive permission mode from the profile and include porcelain in the diff evidence; evidence — `EV-SP-014-20260822-LIVE-ACCEPTANCE-COMPLETED-02`; falsifier — a completed write-capable task whose worktree has no diff verifying true, or a readOnly turn that mutates; residual — codex default model/copilot quota still prevent a live codex/copilot turn on this machine (external, out of scope); why SP-015 safe — SP-014 completion gate met.
- **Acceptance verdict:** SP-014 completion gate ("all live coding scenarios pass with direct evidence") **MET**. **SP-014 `completed`.** SP-015 is safe to start.
...
- **Prompt:** `SP-015` (`AURA_RUNTIME_COMPLETION/prompts/second_pass/SP-015_WAKE_WORD_DECISION_AND_EVALUATION.prompt.md`).
- **Verified start commit:** `389ea344652d3d1d8211e6ce244f909eff42bc6e` (== `origin/main`); worktree clean at start.
- **Objective:** make one evidence-backed decision — qualify a real local wake word or explicitly exclude it from the release scope (SP-015 Procedure step 3).
- **Assumptions / risks:** no licensed local wake candidate is provisioned or bundled; the active authority forbids `download_models`/`install_dependencies`, so no candidate can lawfully be obtained/qualified in this pass. Projection drift risk (ADR-042 file absent) and false-wake-claim risk were the primary risks.
- **Authority:** `edit:true`; `download_models:false`; `install_dependencies:false`; `commit/push/merge/release_or_deploy/mutate_permissions:false`.
- **Exact work:**
  - Created `AURA_RUNTIME_COMPLETION/context/WAKE_MODEL_INVENTORY.md` — records zero provisioned wake-word candidates; the on-disk artifact scan (`find . -name '*.mlmodel' -o -name '*.mlmodelc' -o -name '*.tflite' -o -name '*.onnx' -o -name '*.bin'`) returns only Chatterbox ONNX library conformance fixtures (operator tests), not wake models.
  - Confirmed the truthful UI is already present: `AuraMenuView.swift` ("Activation: Push to Talk"; "A trained acoustic wake-word model is not installed"), onboarding stage `.wakeWord` ("no acoustic model is installed ... Push to Talk remains available"), `AuraAppModel_Runtime.swift` warning ("Acoustic wake-word model unavailable; use Push to Talk").
  - Confirmed production wires `WakeWordPipeline(wakeDetector: DisabledWakeWordDetector())`; `MarkerWakeWordDetector` is test-only (ADR-003).
  - Confirmed **no `ADR-042` file exists** anywhere (the decision register references `docs/decisions/ADR-042-voice-routing-resource-governor.md`, which is absent); ADR-042 stays `Proposed`.
  - Updated `SECOND_PASS_OPEN_GAPS.md` (OPEN-08 wake bullet, history preserved), `EVIDENCE_INDEX.md`, `RISK_REGISTER.md` (`RISK-NO-REAL-WAKE-WORD` → Mitigating — explicitly excluded), `DECISION_REGISTER.md` (ADR-042 path note), `capability-matrix.json` (voice.wake_word evidence + exclusion note), `SECOND_PASS_STATE.json`, `current-state.json`, `session-handoff.json`, `ACTIVE_CONTEXT.md`, `SECOND_PASS_LEDGER.md`, `PROGRAM_LEDGER.md`, `ledger/PROJECT_LEDGER.md`.
- **Decision / result:** **wake word is EXPLICITLY EXCLUDED from the release scope** (SP-015 Procedure step 3). No licensed local candidate is provisioned or bundled, and the active authority forbids `download_models`/`install_dependencies`, so qualification is not lawfully possible in this pass. Production remains Push-to-Talk-only (`DisabledWakeWordDetector`); the truthful UI already states no acoustic model is installed; no wake-word claim is made. Re-evaluation requires the user to grant model-download authority and supply a licensed local candidate with Turkish/FAR-FRR/noise/self-trigger/license-hash/soak evidence.
- **Verification:** `python3 scripts/validate_second_pass_program.py` → `SECOND-PASS VALIDATION PASSED`; `AuraAudioTests` 35/35 (includes `disabledWakeDetectorNeverClaimsProductionActivation`), 0 failed bundles.
- **Evidence / class:** `EV-SP-015-20260822-WAKE-EXCLUSION-01` — manual review + system test + static inventory audit.
- **Acceptance verdict by criterion:** one evidence-backed decision — **PASS** (explicit exclusion with truthful UI and no wake-word claim). SP-015 completion gate — **PASS**. **SP-015 `completed`.** SP-016 is safe to start.
- **Authority boundary:** edit-only for delivery; no commit, push, merge, release, notarize, TCC mutation, provider action, telemetry, or model download. Working tree carries the SP-015 control-plane changes; no commit made.
...
- **Prompt:** `SP-016` (`AURA_RUNTIME_COMPLETION/prompts/second_pass/SP-016_BILINGUAL_STT_QUALITY_AND_VOICE_RECOVERY.prompt.md`).
- **Verified start commit:** live `HEAD == origin/main == 94ee2be355046cab97189764e2a9dfb4f7efd57a` (SP-015); worktree clean at start.
- **Objective:** close live STT quality, microphone, barge-in, echo, device, sleep, and permission-recovery gaps (OPEN-08/R7). This pass closes the deterministic metric/fail-closed slice only; the live legs are blocked by authority/environment.
- **Assumptions / risks:** the user is speech-disabled and no speech-capable operator was authorized; the active authority forbids `mutate_permissions` (Speech Recognition TCC) and `download_models`/`install_dependencies`. The existing `BilingualSpeechRecognitionQualityTests` harness documents that the SwiftPM test helper is a bare binary (no Info.plist) and SIGABRTs (exit 134) if it requests authorization, so the live WER corpus must run inside a bundled host with usage-description keys — not available here.
...
- **Exact work:**
  - Diagnosed a measurement gap: `STTPipeline.Metrics` recorded `firstPartialLatencySeconds`/`lastStableLatencySeconds` but **no turn-end latency**, which the R7/R2 evaluation protocol explicitly requires.
  - Added `Metrics.turnEndLatencySeconds` to `Sources/AuraSTT/STTPipeline.swift`; recorded it on stable emission (equal to `lastStableLatencySeconds`) and reset it to `0` at each new turn (matching `firstPartialLatencySeconds`).
  - Added `Tests/AURAIntegrationTests/SP016TurnEndLatencyTests.swift` (3 deterministic tests): turn-end latency record (activation→stable), cross-turn reset, and non-stable/error-never-promoted-to-stable fail-closed invariant.
  - Updated `SECOND_PASS_OPEN_GAPS.md` (OPEN-08 STT bullet, history preserved), `EVIDENCE_INDEX.md`, `RISK_REGISTER.md` (`RISK-STT-ROUTER-QUALITY`/`RISK-VOICE-RECOVERY-LIVE` notes), `SECOND_PASS_STATE.json`, `current-state.json`, `session-handoff.json`, `SECOND_PASS_LEDGER.md`.
- **Verification:** `swift test --filter SP016TurnEndLatencyTests` → **3/3 PASS**; AuraSTTTests 19/19; AuraAudioTests 35/35; AURAIntegrationTests 78/78; `validate_second_pass_program.py` → `SECOND-PASS VALIDATION PASSED`. **Computer-use live read-only observation** (`EV-SP-016-20260822-LIVE-STATE-OBSERVATION-02`): launched the installed AURA app and read its live Privacy + Recovery tabs — `Microphone: Granted`, `Active speech recognition: Granted`, `Screen observation: Denied`; `stt ready`, `audio ready`, `voice-resources ready (16384 MB)`, `tts ready (Yelda fallback)`, `wake-word unsupported (Push-to-Talk only)`; status `Idle — use Push to Talk`. No TCC change and no live mic turn (`mutate_permissions:false`; operator speech-disabled).
...
- **Acceptance verdict by criterion:** the deterministic metric/fail-closed slice and the live truthful-health readout are closed, but the SP-016 completion gate ("defined bilingual quality and recovery thresholds pass on target hardware or the affected capability is excluded") is **NOT MET**: the live bilingual WER/entity corpus and the hardware recovery matrix (barge-in/echo/device/sleep/TCC/helper-crash) require a speech-capable operator and were not exercised. **SP-016 remains `in_progress`; SP-017 must NOT start.**
- **Residual risks:** `RISK-STT-ROUTER-QUALITY` (live corpus) and `RISK-VOICE-RECOVERY-LIVE` (hardware recovery) remain Open and are not closable under the current authority/environment. Forwarded as blockers to SP-016's live legs.
- **Authority boundary:** edit-only for delivery; no commit, push, merge, release, notarize, TCC mutation, provider action, telemetry, or model download. Working tree carries the two intended product/test edits + control-plane updates; no commit made.
...
## 2026-08-22T22:05:00Z — SESSION CLOSEOUT — AURA-SP-016-BILINGUAL-QUALITY-20260822

- **Actor:** Claude Opus 5 (Claude Code), operator-supervised.
- **Active prompt at close:** SP-016 → **completed**; SP-017 `pending`.
- **Verified start commit:** `94ee2be355046cab97189764e2a9dfb4f7efd57a` (SP-015).
- **Verified end commit:** `3196b6fa9cfed51080271b3780f0b704ffe5d914`.
- **Objective:** unblock SP-016, which the prior session left `in_progress` with its completion gate neither passed nor lawfully excluded, and close it properly.
- **Delivered:** new diagnostic probe `Sources/AuraSpeechQualityProbe/` (+ `Resources/AuraSpeechQualityProbe-Info.plist`, `scripts/run-sp016-speech-probe.sh`, `Package.swift` product/target) that holds a scoped Speech grant and measures the real on-device recognizer over a synthetic bilingual corpus; `Tests/AURAIntegrationTests/SP016BilingualFailClosedTests.swift` (4 tests); the prior attempt's `STTPipeline.Metrics.turnEndLatencySeconds` slice and `SP016TurnEndLatencyTests` landed with it; control-plane updates to `SECOND_PASS_OPEN_GAPS.md` (OPEN-08, history preserved and the earlier verdict corrected in place rather than deleted), `EVIDENCE_INDEX.md`, `RISK_REGISTER.md`, `SECOND_PASS_LEDGER.md`, `PROGRAM_LEDGER.md`, `PROJECT_LEDGER.md`, `SECOND_PASS_STATE.json`, `current-state.json`, `session-handoff.json`, `ACTIVE_CONTEXT.md`.
- **Evidence IDs created:** `EV-SP-016-20260822-BILINGUAL-QUALITY-03` (direct live-system measurement + deterministic regression). Prior-session evidence `EV-SP-016-20260822-TURN-END-METRIC-01` and `EV-SP-016-20260822-LIVE-STATE-OBSERVATION-02` were committed in this pass (previously untracked).
- **Commands actually run:** `AURA_ENABLE_LIVE_SPEECH_TESTS=1 swift test --filter BilingualSpeechRecognitionQualityTests` → `.speechNotAuthorized` (documents the host blocker); `./scripts/run-sp016-speech-probe.sh` → 48 recognitions, report written; `swift test --filter SP016` → **7/7 PASS**; `./scripts/aura-test.sh` → **Failed bundles: 0**; `python3 scripts/validate_second_pass_program.py` → **SECOND-PASS VALIDATION PASSED**.
- **Acceptance verdict by criterion:** *bilingual quality thresholds on target hardware* — **MET for the conversational/command scope** (entity recall 1.000 in all four general/command bands; finalization latency 0.05 s). *Affected capability excluded* — **MET for code-switched English technical tokens**, on a measurement, with the obvious mitigation tested and disproven. *Fail closed / never rewrite a bad transcript into a successful command* — **MET**, locked by regression over the verbatim garbled transcripts. *Recovery matrix* — **NOT met, and not claimed**; carried as an open risk.
- **Blockers / residual risks:** `RISK-VOICE-RECOVERY-LIVE` **Open** — barge-in, echo/self-transcription, device switching, sleep/wake, TCC revocation, and helper-crash recovery are unexercised, and `AuraAudio.handleConfigurationChange` has **zero test coverage** because reaching `state == .running` requires a real `AVAudioEngine` input node and therefore a **Microphone** grant for the test host, outside this session's Speech-only authority; concrete closure path recorded. `RISK-STT-ROUTER-QUALITY` partly closed — human-speech quality (accent, disfluency, real room acoustics, real microphone colouration) remains unmeasured and the synthetic corpus is an **optimistic bound**.
- **Authority boundary:** `edit`, `launch_or_install_app`, **`mutate_permissions` scoped by explicit in-session user grant to one Speech Recognition authorization for a local diagnostic bundle** (no microphone grant, no `tccutil`, no model download, no dependency install, no `/Applications` install), and **`commit`/`push` by explicit user grant**. Merge, release/deploy, signing/notarization, provider accounts, and telemetry remained forbidden and were not exercised. **The scoped TCC, commit, and push authorities expire with this session.**
- **Privacy:** the archived probe report was redacted before commit — the machine hostname was replaced with CPU architecture, and the probe was fixed so it never records a hostname again. No raw audio retained (synthesized files are deleted after recognition); only transcripts of `say`-generated text and aggregate metrics are recorded.
- **Next safe action:** start **SP-017** (TTS, resource soak, ADR-042) under its own authority, carrying `RISK-VOICE-RECOVERY-LIVE` forward.
...
## 2026-08-23T15:32:58Z — SP-016 — Flaky recovery suite stabilized after operator re-verification — **completed**

- **Prompt / gap:** SP-016 / `OPEN-08` (R7). **Evidence:** `EV-SP-016-20260823-FLAKY-RECOVERY-STABILIZATION-05` — deterministic system tests against the real `AuraAudio` actor with live audio hardware, plus repeated-run stability proof.
- **Why this entry exists:** operator re-verification ("kusursuz kapanmadı mı") found that the recovery suite claimed stable in `EV-SP-016-20260822-RECOVERY-MATRIX-04` was **flaky**. Three independent runs: run 1 failed `Sleep suspends capture and wake resumes it`, run 2 failed `A configuration change after stop never reopens the microphone`, run 3 passed; the full suite reported `AuraAudioTests` as the one failing bundle. Appended, not edited — the prior entries stand.
- **Root causes:** (1) **async observer registration race** — `Task { for await }` subscriptions could let `start()` return before subscribing, so a posted notification was dropped forever and the recovery handler never ran; (2) **cross-suite microphone contention** — Swift Testing `.serialized` serializes within one suite only, so `AuraAudioTests` and `SP016DeviceRecoveryTests` both opened the same real `AVAudioEngine` input concurrently.
- **Delivered:** `AuraAudio` now uses **synchronous** `NotificationCenter.addObserver` tokens (`configurationChangeObserver`/`sleepObserver`/`wakeObserver`) with `removeObservers()` in `stop()`; all microphone-opening tests consolidated into one `.serialized` suite; a generous `waitUntil` helper (~15 s + final check) replaced the short fixed poll.
- **Verification:** `AuraAudioTests` (39 tests / 6 suites) passed **six consecutive independent runs**; `./scripts/aura-test.sh` → **21/21 bundles, Failed bundles: 0**; static diagnostics clean; validator PASSED.
- **Acceptance verdict:** SP-016 recovery suite is now **deterministically stable**; SP-016 completion gate stays **MET**. **SP-017 safe to start.**
- **Residual risks:** product recovery behaviour unchanged — `RISK-VOICE-RECOVERY-LIVE` stays **Open** for user-present physical verification only (no headset unplug, no real route change, no real sleep, no acoustic barge-in/echo).
- **Authority boundary:** edit, commit, push granted by user; merge/release/deploy/signing to be handled separately per the user's explicit delivery request.

## 2026-08-23 — SP-017 — Resource-governor idle unload + NLU/reasoning admission + ADR-042 authored — in_progress

- **Prompt / gap:** SP-017 / `OPEN-08` (R7). **Evidence:** `EV-SP-017-20260823-GOVERNOR-IDLE-UNLOAD-01` — deterministic unit/system tests over `VoiceResourceGovernor` and `OllamaAdapter` governor routing.
- **Chatterbox latest verified:** installed runtime = `ResembleAI/chatterbox` rev `5bb1f6ee58e50c3b8d408bc82a6d3740c2db6e18` variant `multilingual-v3`, matching the repo helper/install pin; venv Python 3.11, torch 2.13.0, MPS available, reference WAV + model snapshot present. Repo helper script is current.
- **Why this entry exists:** SP-017 started. Two concrete R7 gaps were found and closed deterministically: (1) `VoiceResourceGovernor` declared `idleUnloadAfterSeconds` (R7-G idle unload) but never implemented it; (2) the local NLU/reasoning workload was not admitted through the shared governor (R7-G + OPEN-08 wording), so it could not fail closed on shared-resident/thermal/pressure denial.
- **Delivered:** `VoiceResourceGovernor` now tracks per-workload `lastActiveAt` and exposes `@discardableResult unloadIdleReservations()`; `start()` runs an `idleUnloadTask` polling every half-window; `stop()` cancels/clears; `reserve`/`release` record activity. `OllamaAdapter` accepts an optional shared `resourceGovernor`, reserves `.reasoning` (2 GB) in `preflight` before admission, and releases on every terminal path of `classify`/`structuredNLU`/`summarize`/`reason`; shared-governor denial degrades `.budgetExceeded` (fail closed) and opens a circuit. The production `OllamaAdapter` is wired to the kernel's shared governor. `ADR-042` authored at `docs/decisions/ADR-042-voice-routing-resource-governor.md` (scope/alternatives/consequences/expiry/evidence) but stays **Proposed** pending explicit user acceptance.
- **Verification:** `VoiceResourceGovernorTests` 7/7 (3 new: idle-window unload, recent survives, reserve-refreshes-activity); `OllamaAdapterTests` 18/18 (2 new: reasoning reserves-and-releases, reasoning fails closed on shared denial); `ChatterboxTTSEngineTests`/`STTRouterTests` pass; `swift build` clean. Full-suite result recorded under `EV-SP-017-20260823-FULL-SUITE-01`.
- **Acceptance verdict:** SP-017 **in_progress** — a bounded, deterministic slice is delivered, but SP-017 is not complete (no commit/push granted for this prompt; live soak/measurement and the full-suite aggregate still to finalize). SP-018 must NOT start.
- **Residual risks:** `RISK-MODEL-MEMORY-PRESSURE` and `RISK-NEURAL-TTS-LATENCY` stay Open with a documented closure path; `screenVision`/`codingAgent` explicitly not admitted through the shared governor (bounded per-capture screen + spawned CLI subprocesses; documented in ADR-042). Measured 16 GB co-resident soak, neural-TTS live first-audio/MPS qualification, and human listening acceptance remain open and belong to later R7 steps.
- **Authority boundary:** SP-017 hard boundaries — no install, launch, TCC, provider, sign, release, deploy, **commit, push, or merge**; all changes are working-tree edits.

### 2026-08-23T14:16:40Z — SP-017 — system-TTS-only release closeout — completed

- **Actor:** Codex, using the repository control contract and the computer-use skill for a read-only live AX observation.
- **Active prompt:** SP-017 / OPEN-08 / R7.
- **Verified start/end commit:** `f6518e1333015b31c4783f0a4e8f033555f6e1f1` / same commit; working-tree edits intentionally remain uncommitted.
- **Objective:** close voice output/resource governance or define a truthful system-TTS-only release.
- **Delivered:** system-only `TTSAdapterChain` release default and regression assertion; accepted ADR-042 for the bounded PTT + system TTS scope; recorded direct live system-TTS and host/resource evidence; updated OPEN-08, performance/TTS documentation, evidence, risks, decisions, state, and handoff projections. Existing governor idle-unload and Ollama reasoning admission changes remain in scope.
- **Evidence:** `EV-SP-017-20260823-LIVE-SYSTEM-TTS-01`, `EV-SP-017-20260823-RESOURCE-SCOPE-02`, `EV-SP-017-20260823-GOVERNOR-IDLE-UNLOAD-01`, and `EV-SP-017-20260823-FULL-SUITE-01`.
- **Acceptance by criterion:** system TTS direct live 14/14 with first chunk 0.733 s and full utterance 1.400 s — pass; resource admission/control tests — pass; neural/wake capabilities — explicitly excluded; PTT + system TTS truthful — pass; ADR-042 alternatives/scope/expiry/evidence — accepted for this bounded scope.
...
### 2026-08-24T07:57:49Z — SP-018 verification correction — test-runner boundary

- **Symptom / missing postcondition:** the default full matrix exposed a
  scheduling-dependent `AuraAgentTests` failure: the specialist-swarm test
  observed two failures and one approval instead of one failure and two
  approvals, while the same test passed in isolation.
- **Mechanism / layer:** unrestricted Swift Testing parallelization let the
  `AuraAgentTests` live CLI, real-worktree, and actor-backed fixtures contend
  under the 21-bundle runner. The defect was in the test-runner boundary, not
  the SP-018 production memory-reference wiring.
- **Resolution:** the production test runner now bounds only `AuraAgentTests`
  to one worker by default, with an explicit width override for controlled
  experiments; a Python regression test and README contract were added.
- **Evidence / result:** `EV-SP-018-20260824-TEST-RUNNER-FIX-06` records the
  pre-fix red regression and corrected green results: regression test passed,
  AuraAgentTests **237/237**, default full matrix **21/21 bundles, 0 failed**,
  exit 0; syntax and diff checks passed.
- **Falsifier / residual:** a fresh width-one default run reproducing the same
  failure would falsify the correction. The toolchain-specific environment
  control does not establish live app, restart, remote/provider, release, or
  deployment evidence; those risks remain open and outside this bounded fix.
- **State / authority:** SP-018 remains the completed local wiring slice;
  SP-019 remains pending. The verification/docs/ledger changes are
  intentionally uncommitted; commit, push, merge, release, and deploy remain
  unauthorized in this correction.

### 2026-08-24T08:01:03Z — SP-018 session closeout — verification correction reconciled

- **Session / actor:** `AURA-SP-018-VERIFICATION-CORRECTION-20260824`; Codex.
- **Active prompt / commits:** SP-018 / OPEN-09 only; verified start and end
  commit `ed55a0c8db9c63059c7639f9160efebaf44816ac` on `main`, with
  `origin/main` equal. The product delivery remains the historical parent
  `1d3efca0944334be19a2d68abbb4c199bba15d87`; the correction remains
  uncommitted by authority.
- **Objective / delivered:** close the observed default-runner defect without
  changing the SP-018 production path. The `AuraAgentTests` bundle is bounded
  to one Swift Testing worker by default; a regression test, README contract,
  evidence, risk, decision, state, and handoff projections were synchronized.
- **Evidence / verification:**
  `EV-SP-018-20260824-TEST-RUNNER-FIX-06`; regression test passed,
  `AuraAgentTests` passed 237/237, the default full runner passed 21/21 bundles
  with zero failures and exit 0, JSON/schema/runtime/second-pass/repo-hygiene
  validators passed, 39 governance tests passed, and syntax/diff checks passed.
- **Acceptance by criterion:** deterministic local verification boundary —
  **met**; SP-018 production reference-wiring behavior — **not changed and
  remains accepted**; live app/restart/provider/remote/security/release/deploy
  acceptance — **outside scope and not claimed**.
- **Blockers / residual risks:** the Swift Testing variable is
  toolchain-specific and must be rechecked after toolchain changes. R8 live
  memory controls, contradiction/restart, ADR-043, R9, remote/provider,
  signing, release, and deployment gates remain open.
- **Authority boundary:** edit-only; no launch, install, TCC, provider,
  signing, commit, push, merge, release, deploy, or external write occurred.
- **Exact next safe action:** SP-019 remains the first pending prompt; a new
  session must read its required control files and prompt before any action.

### 2026-08-24T08:45:49Z — SP-019 attempt — deterministic local controls delivered; live gate open

- **Session / actor:** `AURA-SP-019-ATTEMPT-20260824`; Codex.
- **Active prompt / commits:** SP-019 / OPEN-09 / R8. Verified branch `main`,
  start and end `HEAD == origin/main ==
  ed55a0c8db9c63059c7639f9160efebaf44816ac`; worktree intentionally dirty.
  No commit, push, merge, release, or deployment occurred.
- **Objective / symptom:** close the missing production reachability for
  bounded memory preferences and user-visible inspect/correct/delete/export,
  conflict, and retention controls, then prove restart-safe behavior and the
  eight R8 scenarios. Before this attempt the profile store was not wired into
  the production kernel lifecycle, the Privacy surface lacked the full control
  projection, and user correction supplied no evidence reference.
- **Delivered:** production `AuraKernel` profile-store wiring; runtime APIs for
  profile load/save/clear, conflict snapshots/resolution, superseded-record
  inspection, and retention enforcement; AppModel restart restoration; Privacy
  controls for purpose/scope/retention, search, conflict triage,
  correction/deletion/export, and cleanup; and evidence-backed user correction.
  ADR-043 received an implementation note and remains Proposed.
- **Evidence / verification:** `EV-SP-019-20260824-LOCAL-CONTROLS-01` records
  production build, focused `AURAIntegrationTests` **83/83**, full **21/21
  bundles / 1,141 tests / 0 failed**, second-pass validation, 39 governance
  tests, compile/syntax/diff/formatter/lint checks, and the app artifact hash.
  `EV-SP-019-20260824-LAUNCH-SMOKE-02` records only LaunchServices startup and
  exact-process stop; temporary HOME did not isolate Application Support.
- **Acceptance verdict by criterion:** deterministic local composition and
  control projection — **met**; memory policy tests for provenance,
  contradiction, supersession, correction, deletion, export, retention, audit
  exclusion, and non-weakening — **met locally**; user-present restart,
  verified fact, multi-turn reference, destructive clarification, contradiction
  resolution, live inspect/correct/delete/export, provenance display, and live
  local-only remote exclusion — **not met**. SP-019 completion gate — **not
  met; state remains `in_progress`**.
- **Cognitive gate:** the exact symptom, mechanism/layer, direct resolution,
  evidence classes, falsifier, residual risk, and SP-020 safety answer are
  recorded in `SECOND_PASS_LEDGER.md`; no evidence is promoted from a fake,
  historical line, or model assertion.
- **Blockers / residual risks:** direct user-present operation of all eight R8
  scenarios is missing. The existing R8/R9 live, remote/provider, accessibility,
  ADR-043, signing, release, and deployment risks remain open. The new risk is
  `RISK-SP-019-LIVE-MEMORY-CONTROLS`.
- **Authority boundary:** edit plus the prompt-authorized bounded launch smoke
  only; no install, TCC mutation, provider, remote transport, telemetry/beta,
  signing, commit, push, merge, release, or deploy.
- **Exact next safe action:** user-present manual run of the final local app:
  save a bounded preference, quit/relaunch through LaunchServices, verify
  profile purpose/scope/retention, then run and redact evidence for all eight R8
  scenarios. Revalidate and run the mandatory closeout again. Do not start
  SP-020.

### 2026-08-24T08:45:49Z — SP-019 mandatory session closeout — blocked by live evidence, not source verification

- **Closeout procedure:** `AURA_RUNTIME_COMPLETION/prompts/15_SESSION_CLOSEOUT.prompt.md`
  was read and applied after the attempt, including repository/state/ledger
  review, evidence/risk/decision updates, machine-state reconciliation, and
  schema/validator checks.
- **Closeout result:** SP-019 remains `in_progress`; the evidence stop
  condition is recorded and SP-020 remains unopened. Authority resets to the
  prompt's explicitly granted scope for the next session; no standing launch
  or delivery authority is carried forward.
- **Evidence:** `EV-SP-019-20260824-LOCAL-CONTROLS-01` and
  `EV-SP-019-20260824-LAUNCH-SMOKE-02`, plus closeout reconciliation under
  `EV-SP-019-20260824-CLOSEOUT-03`.

### 2026-08-24T09:42:05Z — SP-019 closeout reconciliation

- **Evidence:** `EV-SP-019-20260824-CLOSEOUT-03`.
- **Final checks:** second-pass, runtime-completion, repo-hygiene,
  supply-chain, JSON/schema, 39 governance tests, and `git diff --check` all
  passed after the synchronized state projections were written.
- **Final verdict:** SP-019 remains `in_progress` under its stop condition;
  direct user-present eight-scenario evidence is missing. Authority resets to
  edit-only and SP-020 remains unopened.

### 2026-08-24T10:50:50Z — SP-019 direct user-present controls attempt

- **Session / actor:** `AURA-SP-019-ATTEMPT-20260824`; Codex; Computer Use was
  explicitly authorized by the user for this attempt.
- **Active prompt / repository:** SP-019 / OPEN-09; `main`,
  `HEAD == origin/main == ed55a0c8db9c63059c7639f9160efebaf44816ac`; worktree
  intentionally dirty and preserved. No commit, push, merge, signing, release,
  or deploy occurred.
- **Objective / procedure:** launched the final app in an isolated Foundation
  home; saved `Concise`, quit through the AURA menu, relaunched, and inspected
  the restored profile; operated correction, retention, audit exclusion,
  export, and local-only controls with redacted observations only.
- **Evidence:** `EV-SP-019-20260824-LIVE-CONTROLS-04` is direct user-present
  partial evidence; `EV-SP-019-20260824-LOCAL-CONTROLS-01` remains the
  deterministic companion.
- **Verdict:** restart persistence, metadata, inspection, correction, retention
  invocation, audit exclusion, and fail-closed remote preference rejection
  were observed. Verified tool fact, resolved reference, destructive
  clarification, contradiction resolution, export artifact, deletion receipt,
  and direct transport trace remain unproven. Overall gate **not met**.
- **Cognitive / residual:** remaining gaps are evidence/provider and
  confirmation boundaries, not claims promoted from tests or labels. ADR-043
  remains Proposed. A located export, conflict resolution, and deletion receipt
  would change the residual verdict; a fresh restart or policy failure would
  falsify the direct subset.
- **Authority / next action:** the disposable Delete control is paused at the
  immediate confirmation boundary. After confirmation, delete only that test
  record, capture the receipt, retry export, and continue SP-019; keep
  `in_progress` and do not start SP-020.
### 2026-08-24T10:58:37Z — SP-019 mandatory session closeout after validator correction

- **Session / actor:** `AURA-SP-019-ATTEMPT-20260824`; Codex.
- **Active prompt / commits:** SP-019 / OPEN-09 / R8 only; verified branch
  `main`; `HEAD == origin/main ==
  ed55a0c8db9c63059c7639f9160efebaf44816ac`; worktree intentionally dirty and
  preserved. No commit, push, merge, signing, release, or deployment occurred.
- **Correction:** `current-state.json.last_evidence_ids` had 51 items and
  failed its schema maximum. One oldest entry was removed from that bounded
  projection; historical ledger wording was not rewritten.
- **Verification / evidence:** `EV-SP-019-20260824-CLOSEOUT-05`. Runtime
  completion validation passed; second-pass validation passed; governance
  unittest discovery passed 39/39; required JSON parsing and `git diff --check`
  passed.
- **Acceptance verdict:** SP-019 remains `in_progress`. The partial live
  subset remains valid, but the verified tool fact, resolved reference,
  contradiction resolution, export artifact, deletion receipt, and direct
  transport trace are not proven. The eight-scenario gate is not met.
- **Blockers / authority:** `RISK-SP-019-LIVE-MEMORY-CONTROLS` remains open;
  Delete is paused for action-time confirmation. Closeout authority resets to
  edit-only; SP-020 remains unopened.
- **Exact next safe action:** obtain immediate confirmation for deleting only
  the disposable test memory row, capture the receipt, retry export, and
  continue the remaining SP-019 scenarios with redacted evidence.
### 2026-08-24T11:15:56Z — SP-019 live export reconciliation

- **Session / actor:** `AURA-SP-019-ATTEMPT-20260824`; Codex with explicit
  Computer Use authority from the user.
- **Procedure / evidence:** Used the final app in the same isolated profile,
  activated the visible Privacy export control, saved through the native Save
  panel, and verified only structural metadata. `EV-SP-019-20260824-LIVE-CONTROLS-06`
  records the procedure and artifact hash.
- **Observed postcondition:** `/tmp/aura-memory-sp019-export.json` exists,
  parses with keys `conflicts`, `generatedAt`, and `records`, contains 203
  records, has no audit field, and the redacted scan found no raw
  audio/screenshot/token/secret marker.
- **Acceptance verdict:** export is **met**. SP-019 overall remains
  `in_progress`; verified tool fact, resolved reference, contradiction
  resolution, deletion receipt, and direct transport trace remain open.
- **Authority / next action:** no permanent deletion was activated. Obtain
  immediate confirmation before deleting only the disposable test row, then
  capture the receipt and continue the remaining SP-019 scenarios. SP-020
  remains unopened.
### 2026-08-24T11:19:13Z — SP-019 mandatory closeout after export evidence

- **Evidence:** `EV-SP-019-20260824-CLOSEOUT-07`.
- **Verification:** runtime-completion validation passed; second-pass validation
  passed; governance unittest discovery passed 39/39; required JSON parsing and
  `git diff --check` passed.
- **State / acceptance:** export evidence is synchronized in the current
  projections, but SP-019 remains `in_progress`. Verified tool fact, resolved
  reference, contradiction resolution, deletion receipt, and direct transport
  trace remain open; SP-020 remains unopened.
- **Authority / next action:** no permanent Delete action was taken. Obtain
  immediate confirmation before deleting only the disposable test memory row,
  capture its receipt, and continue SP-019. No commit, push, merge, signing,
...
- **Delivered changes (cumulative):**
  - Stable onboarding/header accessibility identifiers.
  - Turkish localization of the status pill, capability ready/no-evidence
    detail, and disabled/degraded reason prose.
  - Fixed the `AuraAgentTests` `exit 142` flake (ProcessRunner stdin EOF).
  - Fixed Dynamic Type scaling (relative text styles).
  - Live AX verification of the primary workflows (tabs, header, composer,
    language switch, non-color status, keyboard shortcuts, confirmation
    expiry/focus containment, reduced motion).
...
- **Active prompt:** SP-023 (R10; OPEN-11). **Verdict: COMPLETED** for the bounded authenticated-IPC and real-helper-execution slice.
- **Authority:** user granted edit/commit/push/merge/launch; no TCC mutation, signing, install, or release authority.
...
- **Active prompt:** SP-024 (R10; OPEN-11). **Verdict: COMPLETED** for the bounded network/OAuth/injection-enforcement slice.
- **Authority:** user granted edit/commit/push/merge/launch; no TCC mutation, signing, install, provider-contact, or release authority.
...
### 2026-08-30 — SP-030 — BLOCKED (beta SLOs, scenarios, incidents, independent sign-offs)

- **Session ID:** `AURA-SP-030-BETA-EVIDENCE-20260830`; actor: GitHub Copilot.
- **Active prompt:** SP-030 (R12; OPEN-13). **Verdict: BLOCKED / remains `in_progress`.**
- **Objective:** Run the controlled beta evidence program: compute percentile SLOs (latency, STT/WER, task completion, verification, crash/recovery, accessibility, privacy, unauthorized actions) from an approved sample; run the Turkish/English/mixed scenario matrix plus false-success/unauthorized-action/update/recovery/uninstall cases and incident review; remedy incidents; and obtain independent security/privacy/accessibility/localization/release sign-offs.
- **Why blocked (exact missing postconditions):**
  - Completion gate requires "Mandatory SLOs and scenarios pass, incidents are remediated, and independent sign-offs are complete." None is met.
  - No enrolled/consented beta cohort (`cohort.not_enrolled`, `consent.not_collected`); no approved sample to compute SLOs from; no authority to enroll/consent a participant.
  - No enabled measurement/transport: the content-free aggregate engine (`EV-SP-029-20260830-TELEMETRY-AGGREGATOR-01`) is default-off with `transport: none`; `telemetry.enabled: false`. No live beta window exists to run the scenario matrix or collect SLO/incident data.
  - No independent evaluator: all five sign-offs `not_obtained`; an independent sign-off requires a non-implementing evaluator not present and cannot be fabricated.
  - R11 dependency incomplete (`in_progress`, `development_unverified`, no signed/notarized clean-machine artifact, ADR-046 not accepted; `r11_completion_required: true`).
...
- **Delivered/evidence:** `EV-SP-030-20260830-PROGRAM-BLOCKED-01` (process/blocked) records the exact blocker and cognitive-gate answers. Reconciled stale `current-state.json` repository pointers to live `HEAD == origin/main == 8b16142` (clean worktree).
- **Verification:** `validate_second_pass_program.py` → PASSED; `validate_beta_readiness.py --record ...` → "valid and blocked" (both exit 0). No telemetry transmitted, no cohort enrolled/consented, no SLO measured, no incident reviewed, no sign-off obtained, no RC approved.
- **Falsifier:** a claim that SP-030 is completed, that SLOs/scenarios passed, that incidents were remediated, that an independent sign-off was obtained, that a cohort was enrolled/consented, that telemetry was transmitted, that `beta-readiness.json` left `blocked`, or that SP-031 started would falsify this record.
- **Residual / next action:** `RISK-NO-INDEPENDENT-BETA-EVIDENCE`, `RISK-NO-BETA-CONSENT-BOUNDARY`, `RISK-NO-RC-EVIDENCE-PACKAGE` remain open. Complete the mandatory `15_SESSION_CLOSEOUT.prompt.md`; keep SP-030 blocked/in_progress. SP-031 must NOT start until owner-authorized R11 completion + a real consented beta window + independent evaluation occur and SP-030 is re-run.

### 2026-08-30 — SP-030 — owner broad approval recorded (R11 local gates, ADR-046, beta cohort, SP-031)

- **Session ID:** `AURA-SP-030-BETA-EVIDENCE-20260830`; actor: GitHub Copilot.
- **Active prompt:** SP-030 (R12; OPEN-13). **Verdict: SP-030 remains `in_progress`/blocked; approval recorded as authority, not as live evidence.**
- **Authority event:** The release owner explicitly stated **"neler eksik kaldı ben tümü için onay veriyorum"** in response to the honest inventory of remaining R12/R11 gaps. Recorded as `EV-SP-030-20260830-OWNER-APPROVAL-02`.
- **Unblocks (for a user-present session):** R11 locally-closable gates (live launch-at-login, sleep/wake/crash, safe mode/support-bundle, migration); ADR-046 local-only acceptance; the beta cohort (owner as the single local participant) with the owner's consent; content-free aggregate telemetry for local measurement; and SP-031 (local-only signed RC + ADR-047).
- **Cannot fabricate:** independent sign-offs (need a non-implementing evaluator), live STT/WER (needs a speech-capable operator), and live beta SLO/scenario/incident measurement (needs a user-present session). None produced in this unattended pass.
...
### 2026-08-30 — SP-030 — owner present approval + ADR-046 local-only acceptance

- **Session ID:** `AURA-SP-030-BETA-EVIDENCE-20260830`; actor: GitHub Copilot.
- **Active prompt:** SP-030 (R12; OPEN-13). **Verdict: SP-030 remains `in_progress`/blocked; owner present approval + ADR-046 local-only acceptance recorded.**
- **Authority event:** The release owner, present, stated **"burdayım ve herşeyi onaylıyorum"** on top of the prior broad grant (`EV-SP-030-20260830-OWNER-APPROVAL-02`). Recorded as `EV-SP-030-20260830-OWNER-APPROVAL-03`.
- **ADR-046 local-only acceptance:** ADR-046 advanced from Proposed to **Accepted (local-only scope)** per the R11 closure plan and ADR-049. The local updater/rollback/recovery/safe-mode/reset contract is implemented and adversarially tested (SP-028 `EV-SP-028-20260829-*`); a real externally signed update/transport/distribution remains out of scope and is not claimed. `DECISION_INDEX.md` updated. Evidence: `EV-SP-030-20260830-ADR046-ACCEPTED-01`.
- **Cannot fabricate:** independent sign-offs (need a non-implementing evaluator), live STT/WER (needs a speech-capable operator), and live beta SLO/scenario/incident measurement (needs a user-present beta window). None produced in this pass.
...
### 2026-09-02T09:04:12Z — SP-030 — unattended alternate verification remains insufficient

- **Session / objective:** `AURA-SP-030-UNATTENDED-ALTERNATE-20260902`; attempt to close safely verifiable gaps without the owner present by running the existing lifecycle, integration, STT, and explicitly opt-in synthetic Speech paths.
- **Authority boundary:** Existing local tests and the repository-approved synthetic path only. No TCC/permission mutation, microphone capture, telemetry activation, provider contact, signing, release, deployment, or product-source change.
- **Verification:** `./scripts/aura-test.sh /tmp/aura-sp030-alt-lifecycle-20260902 AuraLifecycleTests` → **48 tests / 10 suites / 0 failures**; `./scripts/aura-test.sh /tmp/aura-sp030-alt-integration-20260902 AURAIntegrationTests` → **111 / 22 / 0**; `./scripts/aura-test.sh /tmp/aura-sp030-alt-stt-20260902 AuraSTTTests` → **19 / 4 / 0**. The opt-in command `AURA_ENABLE_LIVE_SPEECH_TESTS=1 AURA_TEST_TIMEOUT_SECONDS=90 ./scripts/aura-test.sh /tmp/aura-sp030-alt-synthetic-20260902 AuraSTTTests` failed closed in 3 real-recognizer tests with `speechNotAuthorized`; no permission prompt was requested.
- **Evidence / class:** `EV-SP-030-20260902-UNATTENDED-ALTERNATE-01`, deterministic_harness plus failed-closed synthetic_speech attempt. The product source remained unchanged from verified baseline `0bafc4f`; only this evidence and state projection are new.
- **Acceptance verdict:** This narrows the deterministic contract risk but does not measure `ptt_ack`, `stt_partial`, or live dialogue first-token latency, does not exercise microphone hardware or a live beta window, and does not close R11 sleep/wake/crash, safe-mode export, migration, scenario, or incident gates. SP-030 remains **blocked**; `beta-readiness.json` remains blocked; SP-031 must not start.
- **Residual / next safe action:** A separately authorized bundled synthetic host could produce synthetic first-partial timing with explicit provenance, but it still would not be live-beta evidence. To close SP-030, recover the Computer Use service or use owner-present/manual validation for R11 and collect the qualifying live voice/scenario/incident evidence.

### 2026-09-02T10:05:43Z — SP-030 — local-only deterministic scope completed; broader live-beta gate deferred

- **Authority / decision:** The owner explicitly requested that no live test be performed. `ADR-051` records the bounded local-only scope: deterministic lifecycle, integration, STT, safety, state, evidence, and governance validation may close SP-030 locally; user-present beta, microphone/STT, live latency, live R11 recovery, live scenarios, and incident review are excluded and remain open limitations.
- **Evidence:** `EV-SP-030-20260902-LOCAL-ONLY-SCOPE-01` records the scope decision and provenance. The supporting unattended checks passed `AuraLifecycleTests` 48/10/0, `AURAIntegrationTests` 111/22/0, and `AuraSTTTests` 19/4/0; the opt-in synthetic Speech route failed closed with `speechNotAuthorized` in 3 real-recognizer tests without requesting TCC.
- **Acceptance:** SP-030 is complete **only for the local-only deterministic scope**. `beta-readiness.json` remains `blocked`; `R11` remains `in_progress`; live voice SLOs remain unmeasured; `incident_review` remains `not_run`; and no beta, production, or release claim is made. Five R12 sign-offs and launch-at-login evidence remain recorded but do not substitute for the excluded live gates.
- **Downstream / falsifier:** SP-031 is now the active prompt but remains **blocked and must not execute** because the release candidate package, ADR-047 approval, live prerequisites, and beta readiness are absent. Any future live-beta or external-release scope requires a new explicit scope decision and fresh evidence; deterministic or synthetic evidence must not be relabeled as live.

### 2026-09-02T11:23:09Z — SP-031 — opened in_progress for local-only RC package preparation

- **Scope authority:** The owner wants the linear chain to advance without live testing. ADR-052 amends ADR-051 item 4 only to allow bounded SP-031 package preparation; no release approval follows.
- **Objective / exact work:** Open SP-031 after SP-030 local-only completion and prepare to bind the existing local `development_unverified` artifact, hashes, SBOM/manifests, deterministic test/CI evidence, limitations, rollback plan, and draft ADR-047 decision.
- **Cognitive gate:** Symptom = SP-031 was incorrectly left downstream blocked after predecessor completion; mechanism = SP-030 scope transition updated `completed_prompts` but retained the old downstream guard; direct correction = ADR-052 plus state, handoff, prompt, and manifest projection to SP-031 / `in_progress`; falsifier = any state disagreement or live/release claim from this opening; residual = package, ADR-047, and explicit approval are absent.
- **Evidence / class:** `EV-SP-031-20260902-OPENED-LOCAL-ONLY-01`, process/scope transition; no RC approval.
- **Acceptance / next action:** SP-031 is open / `in_progress`; SP-032 does not start. `beta-readiness.json` and `release_candidate` remain blocked; no live test, beta, production, signing, notarization, or external-release claim.

### 2026-09-02T12:10:20Z — SP-031 local-only `development_unverified` package attempt

- **Objective:** Assemble one provenance-bound local-only package and prepare
  ADR-047 without manufacturing live beta or release evidence.
- **Authority / scope:** Local build/test/evidence and control-plane edits only.
  No live test, microphone/TCC mutation, telemetry/provider contact,
  signing/notarization, publication, deployment, commit, push, merge, or
  release action occurred.
- **Result:** From a clean detached worktree at
  `bee334782262089fa117124ababa9b3c6dfed394`, the package build ran twice and
  produced byte-identical archives. Artifact SHA-256:
  ...
  Release label remains `development_unverified`; signature and notarization
  fields remain false/not submitted/not performed.
- **Evidence:** `EV-SP-031-20260902-LOCAL-ONLY-PACKAGE-01`; the deterministic
  suite passed 1325 tests / 87 suites / 22 bundles / 0 failures with 70.20%
  line coverage. Existing CI evidence was not relabeled to this commit.
- **Cognitive / acceptance result:** The missing package postcondition is
  resolved for assembly and provenance, but independent declared-scope review,
  explicit owner local-only approval, and ADR-047 acceptance are absent. The
  external model manifest is absent, so neural voice is excluded from qualified
  scope. No beta, production, RC, or live SLO claim follows.
- **State / next action:** Keep SP-031 `in_progress`; keep
  `beta-readiness.json` and `release_candidate` blocked and unapproved. Obtain
  the required review/owner decision, then reassess SP-031; do not start
  SP-032.

### 2026-09-02T11:37:18Z — SP-031 — mandatory session closeout, still in_progress

- **Actor / prompt:** Codex; SP-031 / OPEN-13. Verified start and end are `main`, `HEAD == origin/main == bee334782262089fa117124ababa9b3c6dfed394`; the worktree is intentionally dirty from the bounded state, prompt, evidence, ledger, and ADR changes.
- **Objective / delivered:** Reconcile the SP-030 local-only completion into an honest SP-031 opening, preserve the no-live-test boundary, synchronize all projections, and record the exact next action. Added ADR-052 and SP-031 opening/closeout evidence; no product-source workaround or release action was added.
- **Verification / evidence:** `EV-SP-031-20260902-OPENED-LOCAL-ONLY-01` and `EV-SP-031-20260902-CLOSEOUT-LOCAL-ONLY-01`; 64 Python tests and runtime, second-pass, beta-readiness, hygiene, supply-chain, JSON, projection, and diff validators passed.
- **Acceptance verdict / residual:** SP-031 is correctly `in_progress`, not completed. The local `development_unverified` package, ADR-047 decision, and explicit local-only approval remain absent; `beta-readiness.json` and `release_candidate` remain blocked; SP-032 is not safe to start.
- **Authority boundary:** No live test, microphone/TCC mutation, telemetry activation, provider contact, signing, notarization, publication, deployment, commit, push, merge, beta, production, or release approval occurred.
- **Exact next safe action:** Bind the existing local artifact, hashes, SBOM/manifests, deterministic test/CI evidence, limitations, rollback plan, and draft ADR-047 decision; then reassess SP-031's own completion gate.

### 2026-09-02T12:20:09Z — SP-031 — mandatory closeout after package assembly

- **Session / actor:** `AURA-SP-031-LOCAL-ONLY-CLOSEOUT-PACKAGE-20260902`;
  Codex; SP-031 / OPEN-13; `main`; verified `HEAD == origin/main ==
  bee334782262089fa117124ababa9b3c6dfed394`; worktree `dirty_expected`.
- **Objective / delivered:** Applied the mandatory 15_SESSION_CLOSEOUT after
  the bounded local-only package attempt. The package is bound to the clean
  source commit and ADR-047 is drafted `Proposed`; no product source change or
  live/release action occurred.
- **Evidence / verification:** `EV-SP-031-20260902-LOCAL-ONLY-PACKAGE-01` and
  `EV-SP-031-20260902-CLOSEOUT-LOCAL-ONLY-PACKAGE-01`; second-pass,
  runtime-completion, beta-readiness, JSON, release-manifest, archive
  comparison, 64-test Python governance, and `git diff --check` checks passed.
- **Acceptance / cognitive gate:** Assembly/reproducibility is satisfied. The
  symptom was the missing current package/decision postcondition; the root
  cause was a missing current clean source-to-artifact projection in the
  control-plane layer; the direct procedure was two clean builds, manifest
  validation, `cmp`, and full deterministic testing. A hash, commit,
  reproduction, test, or validator mismatch would falsify it. Approval/review,
  ADR acceptance, model manifest, and live gates remain unsatisfied.
- **Authority / next safe action:** Authority is reset to edit/test/state-only.
  No live beta, microphone/TCC, telemetry, provider, signing, notarization,
  publication, deployment, commit, push, merge, production, or release action
  was performed. Obtain independent declared-scope review and explicit owner
  approval for local-only `development_unverified` use, then reassess SP-031.
  Keep `beta-readiness.json` and `release_candidate` blocked and do not start
  SP-032.

### 2026-09-02T12:30:00Z — SP-031 review packet prepared; owner decision pending

- **Scope / authority:** OPEN-13 only; documentation and state projection.
  No live beta, microphone/TCC, telemetry, provider, signing, notarization,
  publication, deployment, commit, push, merge, or release action occurred.
- **Delivered:** `docs/operations/SP-031_LOCAL_ONLY_PACKAGE_REVIEW_PACKET.md`
  binds the exact package evidence, hashes, source commit, deterministic
  evidence, limitations, falsification checks, independence disclosure, and
  approve/return decision template. Evidence:
  `EV-SP-031-20260902-REVIEW-PACKET-01`.
- **Disposition:** The packet is review preparation only. Codex cannot
  self-review; independent declared-scope review, explicit owner local-only
  approval, and ADR-047 acceptance remain absent. SP-031 stays `in_progress`,
  beta-readiness and `release_candidate` stay blocked, and SP-032 does not
  start.

### 2026-09-02T13:05:06Z — SP-031 mandatory closeout after review-packet attempt

- **Evidence:** `EV-SP-031-20260902-CLOSEOUT-REVIEW-PACKET-01` closes out the
  review-packet preparation attempt. Second-pass, runtime-completion,
  beta-readiness, 64-test Python governance, release-manifest, JSON,
  archive-comparison, SHA-256, and diff checks passed.
- **Scope / result:** The review instrument is now linked into the package
  records and is reproducible as a procedure. This remains local process
  evidence only; no live, signing, release, or deployment action occurred.
- **Disposition:** Independent declared-scope review, explicit owner
  local-only approval, and ADR-047 acceptance remain absent. SP-031 stays
  `in_progress`; beta-readiness and `release_candidate` stay blocked; SP-032
  does not start.

### 2026-09-02T14:34:46Z — SP-031 owner decision; local-only package scope completed

- **Decision evidence:** `EV-SP-031-20260902-OWNER-LOCAL-ONLY-APPROVAL-01`
  records the release owner's independent declared-scope review and approval of
  the exact package/manifest hashes for local `development_unverified` use
  only, including acceptance of limitations and ADR-047 at that scope.
- **Cognitive record:** The missing postcondition was owner approval, caused by
  a governance decision gap rather than product behavior. The direct resolution
  was review of the exact evidence and falsification checklist. A hash,
  provenance, validator, or decision-scope mismatch would falsify the result.
- **State / residual:** SP-031 is completed for the narrow local package scope;
  R12 remains `in_progress`. No beta, production, `release_candidate`, signed,
  notarized, or external-release approval exists. `beta-readiness.json` and
  `release_candidate` remain blocked. SP-032 is state-projected but blocked and
  unexecuted because its own direct FINAL gates remain absent.

### 2026-09-02T14:41:08Z — SP-031 mandatory closeout after owner approval

- **Evidence:** `EV-SP-031-20260902-CLOSEOUT-OWNER-APPROVAL-01` records the
  required closeout, exact source/remote equality, dirty-expected boundary, and
  passed validators/tests/package-integrity checks.
- **Handoff:** SP-031 is closed only at local package scope. R12 remains in
  progress; `beta-readiness.json` and `release_candidate` remain blocked.
  SP-032 is blocked and unexecuted, awaiting separate authority and direct FINAL
  gate evidence.

### 2026-09-02T15:07:09Z — SP-032 — STARTED (blocked FINAL reconciliation)

- **Session ID / actor:** `AURA-SP-032-FINAL-RECONCILIATION-20260902`; Codex.
- **Verified baseline:** `main`; `HEAD == origin/main ==`
  `bee334782262089fa117124ababa9b3c6dfed394`; 35 dirty expected control-plane
  paths. The `current-state.json` projection omits the modified
  `AURA_RUNTIME_COMPLETION/state/DECISION_REGISTER.md`; repair that projection
  before any acceptance conclusion.
- **Objective:** perform only the requested OPEN-14 evidence reconciliation and
  blocked final-acceptance audit. Reconcile active capability, decision, risk,
  evidence, handoff, and support claims against direct state; run only safe
  deterministic validation; preserve the FINAL gate as blocked unless direct
  evidence and explicit authority genuinely pass.
- **Authority / assumptions:** the user's request authorizes this bounded
  edit/test/state reconciliation, not installation or launch, TCC mutation,
  provider/beta/telemetry activity, signing, release/deploy, commit, push, or
  merge. ADR-047 approves only the exact local `development_unverified`
  package; ADR-049 permanently scopes external distribution out.
- **Risks / acceptance criteria:** stale prose could overstate R11/R12 or
  external-distribution requirements; deterministic checks could be mistaken
  for clean-Mac or live acceptance. Completion of this attempt requires an
  evidence-backed blocked verdict, current projections that distinguish scoped
  external gates from missing local lifecycle/live gates, passing validators
  and relevant deterministic tests, a complete cognitive record, and the
  mandatory closeout. It cannot set `release_candidate_verified`, `released`,
  or advance SP-033.

### 2026-09-02T15:16:37Z — SP-032 FINAL reconciliation — blocked result

- **Evidence / result:** `EV-SP-032-20260902-FINAL-RECONCILIATION-01`; `main`;
  `HEAD == origin/main == bee334782262089fa117124ababa9b3c6dfed394`;
  `dirty_expected`. Current projections now distinguish accepted local scopes
  (ADR-046/047) and excluded external distribution (ADR-049) from still-missing
  local lifecycle/live FINAL evidence.
- **Cognitive gate:** (1) symptom: no direct FINAL postcondition and stale
  active prose; (2) mechanism: control-plane/evidence projection drift, not a
  product-agent/context defect; (3) resolution: bounded state/document/risk/
  capability/handoff reconciliation; (4) proof: the named manual-reconciliation
  plus deterministic-integrity evidence; (5) falsifier: validator/hash/source
  disagreement, absent claimed direct evidence, or status promotion; (6)
  residual: R2-R10 direct evidence, R11 lifecycle/clean-profile evidence, and
  R12 live SLO/scenario/incident/RC evidence; (7) SP-033: unsafe while SP-032
  stays blocked.
- **Authority / verification:** No launch/install, TCC, provider, beta,
  telemetry, signing, release/deploy, commit, push, or merge occurred. Existing
  package integrity and permitted validators passed but prove neither clean-Mac
  nor release acceptance.
- **Exact next action:** Return each missing postcondition to its owning R2-R12
  track under separate authority; keep SP-032 blocked and do not start SP-033.

### 2026-09-02T15:30:16Z — SP-032 mandatory closeout — blocked handoff

- **Evidence / verification:** `EV-SP-032-20260902-CLOSEOUT-02`; `main`; `HEAD
  == origin/main == bee334782262089fa117124ababa9b3c6dfed394`; dirty_expected.
  Reviewed the bounded diff and reran permitted second-pass, runtime,
  beta-readiness, release-manifest, JSON, SHA-256/archive, and diff checks.
- **Result:** Closeout process passed without a FINAL status promotion. It does
  not substitute for the missing direct R2-R12 gates; the full wrapper/Python
  governance harness remains intentionally not run under this authority.
- **Next action:** Keep SP-032 blocked and return direct evidence collection to
  owning R2-R12 tracks. Do not start SP-033.

### 2026-09-02T16:11:57Z — SP-032 autonomous deterministic-control leg

- **Session / actor:** `AURA-SP-032-DETERMINISTIC-EVIDENCE-20260902`; no
  operator present (autonomous review for later owner inspection).
- **Evidence / verification:** `EV-SP-032-20260902-DETERMINISTIC-SUITE-01`;
  `main`; `HEAD == origin/main == bee334782262089fa117124ababa9b3c6dfed394`;
  dirty_expected. Re-ran `validate_second_pass_program.py`,
  `validate_runtime_completion.py --ci`, `validate_beta_readiness.py --record`,
  and `validate_release_manifest.py`; rechecked SP-031 artifact+manifest SHA-256;
  ran a fresh full `./scripts/aura-test.sh /tmp/aura-sp032-full-20260902`: 22
  bundles, 0 failed, 1325 tests / 87 suites, exit 0 (incl. AuraLifecycleTests 48).
- **Result / class:** All deterministic controls green; recorded as
  `deterministic_harness` only (ADR-051/ADR-052). This does not close FINAL and
  does not promote any synthetic/deterministic result to a live/release class.
- **Next action:** SP-032 remains `blocked`; `beta-readiness.json` and
...
- **Session / actor:** `AURA-SP-032-OWNER-LIVE-ACCEPTANCE-20260903`; owner present
  with full-authority grant.
- **Evidence / verification:** `EV-SP-032-20260903-LIVE-ACCEPTANCE-01`; built
  `AURA.app` + helpers + Safari ext; local-signed `AURA Stable Local Signing`,
  `codesign --verify --deep --strict` exit 0; live launch stable ≥13s with no
  crash and clean quit; fresh full suite 22 bundles/0 failed/1325 tests, line
  coverage 70.19%; supply-chain + repo-hygiene PASSED, 64 Python governance OK;
  BTM shows launch-at-login registered.
- **Result / class:** live local + `deterministic_harness` + automated
...
- **Session / actor:** `AURA-SP-032-OWNER-LIVE-ACCEPTANCE-20260903`; owner
  ...
  SP-033.
- **Evidence / verification:** `EV-SP-032-20260903-OWNER-ACCEPTED-GAPS-01`.
  R11 `dependency_gate.r11_state` → `completed` (local scope, evidence
  ...
  `in_progress` (active).
- **Result / class:** owner decision / accepted-known-gap. No live/clean-Mac/
...
- **Session / actor:** `AURA-SP-033-FINAL-CLOSEOUT-20260903`; terminal second-pass SESSION_CLOSEOUT.
- **Prompt / gap:** `SP-033` / `OPEN-15`; `next_prompt: none`.
- **Verified state:** `main`; `HEAD == origin/main == 44f41c7986445526fd3f40f36c5a3972d26f65ea`; worktree clean.
- **Evidence / verification:** `EV-SP-033-20260903-FINAL-CLOSEOUT-01`; `python3 scripts/validate_second_pass_program.py` PASSED. Recorded exact branch/HEAD/remote/worktree/authority; answered the cognitive completion gate for the entire chain; validated manifest/state/prompt/gap/dependency/evidence/projection references; updated OPEN-15, evidence index, risk register, and all three ledgers.
- **Result / class:** closeout/process + deterministic governance. The chain SP-000–SP-032 is completed for its declared local scopes; SP-033 is the terminal prompt and remains the active prompt in a `blocked` state because the validator structurally requires an active uncompleted prompt and the broader program gates remain open. The chain is **truthfully blocked** with a complete maintainer handoff; no ambiguous state remains.
- **Residual / next action:** R2–R10 direct, R12 live SLO/scenario/incident/RC, and FINAL authority remain open and belong to their owning tracks; `beta-readiness.json`/`release_candidate` stay blocked. No next SP prompt exists; return each open postcondition to its owning R2–R12 track under separate authority.

### 2026-09-03 — SP-033 ADR-053 synthetic-accepted completion — chain COMPLETE

- **Prompt / gap:** `SP-033` / `OPEN-15`; `next_prompt: none`.
- **Scope decision:** `docs/decisions/ADR-053-live-evidence-synthetic-scope.md` (Accepted, 2026-09-03). The user declared live-user acceptance not required and authorized closing every gate blocked solely on absent live evidence with synthetic/deterministic/local-observed evidence at its true evidence class.
- **Evidence / verification:** `EV-SP-033-20260903-SYNTHETIC-ACCEPTED-01`; `validate_second_pass_program.py` PASSED; unit tests PASSED. `SP-033` moved to `completed`; `SECOND_PASS_STATE.json` `program_status=completed`, `active_state=completed`, `blocked_prompts=[]`; handoff/context/ledger/index/decision/risk projections synchronized.
- **Result / class:** scope-decision + process + deterministic governance. The chain SP-000–SP-033 is COMPLETE for the synthetic-accepted local scope. No synthetic evidence is relabeled as live/beta/signed/notarized/production.
- **Residual / boundary:** `beta-readiness.json` / `release_candidate` remain `blocked`/`approved:false` (`release_or_deploy:false`; ADR-049 no Developer ID/notarization/external distribution). External distribution, if ever needed, requires a new ADR. Falsifier = any relabeling of synthetic evidence or release-authority claim.
