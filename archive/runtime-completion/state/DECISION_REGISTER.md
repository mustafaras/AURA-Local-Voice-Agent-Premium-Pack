...
| ADR-042 | Real wake-word engine, STT router, TTS chain, and local model resource governor | R7 | Accepted | System-TTS-only release; future neural/wake enablement requires revisit | `docs/decisions/ADR-042-voice-routing-resource-governor.md` | Must fit 16 GB hardware and preserve Push-to-Talk fallback. **2026-08-22 note:** the referenced ADR-042 file did **not exist** in the repo (projection gap); SP-015 explicitly excludes wake word from the release scope (`EV-SP-015-20260822-WAKE-EXCLUSION-01`). **2026-08-23 reconciliation (SP-017):** the ADR-042 file is authored with alternatives, bounded scope, consequences, expiry/revisit, and evidence. The user accepted the truthful system-TTS-only branch: PTT + system Yelda is the only qualified release path; neural TTS/reference voice, wake word, MPS/CPU neural qualification, human listening, and physical acoustic echo remain explicitly excluded. Evidence: `EV-SP-017-20260823-LIVE-SYSTEM-TTS-01`, `EV-SP-017-20260823-RESOURCE-SCOPE-02`. |
...
| ADR-046 | Signed update, rollback, downgrade protection, safe mode, and recovery | R11 | Accepted (local-only scope) | Direct local lifecycle acceptance | `docs/decisions/ADR-046-signed-update-recovery.md` | The local updater/rollback/recovery/safe-mode/reset contract is accepted under the owner-approved local-only scope (`EV-SP-028-20260829-LIFECYCLE-IMPLEMENTATION-01`, `EV-SP-028-20260829-RUNTIME-API-02`, `EV-SP-030-20260830-ADR046-ACCEPTED-01`). This is not proof of a live update transport or the remaining local lifecycle acceptance: sleep/wake/crash recovery, safe-mode export, populated-profile migration, update/rollback, uninstall/factory-reset, and support-bundle privacy still need direct evidence. |
| ADR-047 | Beta evidence, SLOs, release-candidate authority, and final completion declaration | R12 | Accepted (local-only scope) | Future beta/external-release decision | `docs/decisions/ADR-047-beta-slos-release-authority.md` | The owner independently reviewed the declared scope and approved exactly the SP-031 artifact and manifest hashes for local `development_unverified` use only (`EV-SP-031-20260902-OWNER-LOCAL-ONLY-APPROVAL-01`). This does not approve beta, production, `release_candidate`, signing, notarization, or external release. |
...
| ADR-049 | Local-only distribution: Developer ID and notarization permanently out of scope | R0/R11/R12 | **Accepted** (permanent, owner-instructed) | Local-only release | `docs/decisions/ADR-049-local-only-no-developer-id.md` | The release owner decided AURA is local-only and will never acquire or use Developer ID/notarization. Local nested-signing procedure + hardened runtime + artifact hashes + launch evidence are the local-only release baseline. `RISK-NOT-NOTARIZED` accepted (permanent). External distribution requires a new ADR. 2026-08-30. |
| ADR-050 | Independent review model for the local-only acceptance evidence | R12 | Accepted (local-only scope) | Local-only release | `docs/decisions/ADR-050-independent-review-model.md` | Independent evaluator sign-off is scoped to local-only, not external. |
| ADR-051 | SP-030 local-only deterministic validation scope | R12 | Accepted (local-only scope) | SP-030 completion | `docs/decisions/ADR-051-sp030-local-only-validation-scope.md` | Deterministic/synthetic evidence is provenance-bound; live-beta/R11/incident evidence deferred. |
| ADR-052 | SP-031 local-only RC evidence package scope | R12 | Accepted (local-only scope) | SP-031 completion | `docs/decisions/ADR-052-sp031-local-only-rc-package-scope.md` | Local-only `development_unverified` package preparation; beta-readiness/release_candidate stay blocked. |
| ADR-053 | Live-evidence requirement lift and synthetic-accepted scope | R0–R12/FINAL | **Accepted** (2026-09-03, owner-instructed) | Second-pass FINAL closeout | `docs/decisions/ADR-053-live-evidence-synthetic-scope.md` | User declared live-user acceptance not required; gates blocked solely on absent live evidence are closed with synthetic/deterministic/local-observed evidence. No synthetic evidence is relabeled as live/production; no external signed-and-notarized release is claimed. `release_candidate` remains `approved: false`. |
| ADR-055 | Owner-directed local capability enablement | R0–R12 (local posture) | **Accepted** (2026-09-07, owner-instructed) | Local capability gates | `docs/decisions/ADR-055-owner-directed-local-enablement.md` | Owner-directed: computer use enabled with an open allowlist (all applications; `ownerOpenApplications`), the destructive lifecycle set + `task.delete` seeded without a confirmation challenge (owner-instructed local risk acceptance, not transferable), Ollama cloud inference enabled with an `.always` confirmation grant, `screen.enabled` default `true`, browser host scope all-hosts. Wake word, Safari extension trust, and provider OAuth remain excluded/provisioning steps; ADR-049 external distribution unchanged. Evidence: `EV-ADR-055-20260907-OWNER-ENABLEMENT-01`. |
