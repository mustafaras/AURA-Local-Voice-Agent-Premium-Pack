...
# SP-023 — Authenticated IPC and Privilege Separation

## Mission

Replace application-only envelopes/echo helpers with a real authenticated and least-privilege execution boundary.
...
1. Choose and document authenticated XPC or reviewed equivalent peer identity; bind actor/capability/plan/payload/freshness to verified process identity.
2. Move real Accessibility/generated-input/CLI powers into correctly entitled helpers where required and constrain the main process.
3. Test compromised parent/helper, replay, downgrade, identity mismatch, emergency stop, and helper crash containment.
4. Do not claim separation from a typed pipe or sandbox attestation alone.
...
- Why is SP-024 now safe to start?

## Required records

- Evidence ID prefix: `EV-SP-023`; include timestamp, branch/commit, command or procedure, environment, result, artifact path/hash, scope, and limitations.
...
Peer identity, real helper execution, entitlement scope, and compromise containment are independently evidenced.

## Stop condition

If any required evidence, authority, cognitive answer, postcondition, or validator result is missing, keep SP-023 `in_progress` or `blocked`, record the exact blocker, and do not proceed to SP-024.
