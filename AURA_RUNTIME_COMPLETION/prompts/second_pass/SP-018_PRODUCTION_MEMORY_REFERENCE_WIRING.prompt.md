...
# SP-018 — Production Memory Reference Wiring

## Mission

Populate bounded, provenance-aware reference candidates through the real production composition path.
...
- `reference resolver`
- `DialogueEngine`
- `recent files/tools/workspaces/tasks`
...
1. Connect salience from dialogue, recent files, tools, workspaces, durable tasks, and backend identity into ContextBuilder.
2. Resolve references such as “that repo,” “last file,” and “previous test” only with sufficient evidence.
3. Force clarification for ambiguity or destructive targets and re-run policy/postcondition checks before mutation.
4. Add tests for scope isolation, authority ranking, expiry, and candidate omission.
...
- Why is SP-019 now safe to start?

## Required records

- Evidence ID prefix: `EV-SP-018`; include timestamp, branch/commit, command or procedure, environment, result, artifact path/hash, scope, and limitations.
...
The live production path can resolve safe references with provenance and clarifies every unsafe ambiguity.

## Stop condition

If any required evidence, authority, cognitive answer, postcondition, or validator result is missing, keep SP-018 `in_progress` or `blocked`, record the exact blocker, and do not proceed to SP-019.
