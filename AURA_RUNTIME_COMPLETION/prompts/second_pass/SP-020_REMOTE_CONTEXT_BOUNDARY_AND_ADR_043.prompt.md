...
# SP-020 — Remote Context Boundary and ADR-043

## Mission

Either prove a redacted, user-approved remote-context path or keep local-only mode as the explicit product boundary.
...
1. Inventory every remote context path and prove local-only mode sends no unapproved memory, secrets, raw audio, screenshots, or prompts.
2. If remote context is enabled, implement redaction, allowlist, consent, transport capture, negative-transmission tests, and provider/account isolation.
3. Measure context budget/latency/soak and document fail-closed behavior on transport failure.
4. Accept ADR-043 only with the chosen scope, alternatives, retention, and review date.
...
- Why is SP-021 now safe to start?

## Required records

- Evidence ID prefix: `EV-SP-020`; include timestamp, branch/commit, command or procedure, environment, result, artifact path/hash, scope, and limitations.
...
Remote delivery is independently proven safe or explicitly excluded; local-only claims remain truthful.

## Stop condition

If any required evidence, authority, cognitive answer, postcondition, or validator result is missing, keep SP-020 `in_progress` or `blocked`, record the exact blocker, and do not proceed to SP-021.
