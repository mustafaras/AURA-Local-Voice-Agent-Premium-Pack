...
- **Residual risks:** no blockers. `RISK-NO-REAL-WAKE-WORD` stays Mitigating — explicitly excluded; re-enablement requires future model-download authority and evidence. `ADR-042` file is absent (projection gap) and stays `Proposed`. Forwarded: bilingual STT quality/recovery (SP-016 scope), wake FAR/FRR etc. (excluded, not owned by SP-015).
