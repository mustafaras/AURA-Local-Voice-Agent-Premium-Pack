...
| RISK-INDIRECT-PROMPT-INJECTION | Web/mail/document/screen content can attempt to redirect tools or policy. | R4/R5/R10 | High | Critical | Mitigating — reduced | R5 external page/mail/event/contact content is typed with non-authoritative provenance, scanned by `PromptInjectionClassifier`, and blocked on direct injection fixtures; content cannot influence actions structurally. The Gmail leg now also has direct live injection-blocking and no-leakage evidence. General R10 coverage and live browser/document acceptance remain open. Evidence: `EV-R5-20260808-READ-FIRST-ADAPTERS-01`, `EV-SP-011-20260819-LIVE-GMAIL-CLOSEOUT-07`. **SP-024 (2026-08-27):** added model tool-spoof (system-message and fake-tool-call) and indirect-injection (mail body, repository file, terminal output) adversarial cases plus `PromptInjectionScreen` withhold/pass-through cases. Evidence: `EV-SP-024-20260827-NETWORK-OAUTH-INJECTION-01`. Still open: live browser/document acceptance. |
...
| RISK-MAIN-PROCESS-PRIVILEGE-CONCENTRATION | Main process combines Accessibility, generated input, CLI, models/network, and UI. | R10 | High | Critical | Mitigating — reduced | Typed/hash-bound/replay-protected helper envelopes and helper-kind capability allowlists are implemented, but helpers are parent-launched echo boundaries and the main process still retains broad authority. Authenticated peer transport, real helper execution, entitlements, and compromise-boundary evidence remain required. Evidence: `EV-R10-20260809-BOUNDARY-SLICE-01`. |
| RISK-NETWORK-ALLOWLIST-INCOMPLETE | Allowlist object may not be enforced by every network path. | R10 | Medium | Critical | Mitigating — reduced | Endpoint scheme/host/port/path checks and redirect rejection cover the Ollama loopback client; a mandatory client factory, DNS/IP revalidation, TLS/proxy/download bounds, provider transport, and subprocess audit remain open. Evidence: `EV-R10-20260809-BOUNDARY-SLICE-01`. **SP-024 (2026-08-27):** added `URLSessionFactory` (deny-by-default cookies/cache/redirect) and routed both production `URLSession` clients (`URLSessionProviderFetcher`, `URLSessionOllamaAPIClient`) through it; added `ResolvedIPValidator` (resolved-IP allowlist, DNS-rebinding defense). Evidence: `EV-SP-024-20260827-NETWORK-OAUTH-INJECTION-01`. Still open: a live resolver seam for any non-loopback capability, TLS/proxy/download bounds, and a subprocess network audit. |
| RISK-IPC-PEER-AUTH-ABSENT | A typed pipe envelope can be replay/tamper resistant without authenticating the OS peer that supplied it. | R10 | Medium | Critical | Mitigating — reduced | **SP-023 (2026-08-27):** added authenticated peer identity — `HelperIPCAuthenticator` (HMAC-SHA256 tag over exact transmitted bytes) plus `SecCodeHelperIPCPeerVerifier` (designated-requirement process verification, the reviewed equivalent to XPC peer identity) — and `HelperIPCClient` (SHA-256 digest + peer identity + replay/freshness/capability allowlist + output/time bounds). Shell and automation helpers now execute real typed operations and verify/sign the HMAC tag. Adversarial tests cover compromised/missing helper, replay, protocol downgrade, identity mismatch, helper crash containment, capability escalation, and forged/misbound responses. **Still open:** a live signed-helper launch with a provisioned Keychain secret, OS-confinement attestation of a real signed bundle, and full privilege separation of every privileged path. Evidence: `EV-SP-023-20260827-AUTHENTICATED-IPC-01`. |
| RISK-OAUTH-LIFECYCLE-INCOMPLETE | OAuth contract and Keychain expiry handling exist, but provider transport, callback isolation, live revocation, and leakage coverage are incomplete. | R10 | Medium | Critical | Open — reduced | Gmail now has a PKCE/state-bound loopback exchange, approved-account probe, Keychain enrollment, user-reachable connect control, live local/provider revocation, and immediate fail-closed post-revoke evidence. Direct Privacy-tab automation was not observed because the Computer Use native pipe closed on that SwiftUI tab, and cross-provider/expiry/support-path leakage coverage remains open. Evidence: `EV-R10-20260809-BOUNDARY-SLICE-01`, `EV-SP-011-20260819-LIVE-GMAIL-CLOSEOUT-07`. **SP-024 (2026-08-27):** added `googleOAuthAccessToken`/`googleOAuthRefreshToken` to the canonical `SecretPatternLibrary` and an OAuth leakage corpus proving token material never reaches a reference, a diagnostic, a redacted summary, or a Keychain key, and that revocation deletes the material. Evidence: `EV-SP-024-20260827-NETWORK-OAUTH-INJECTION-01`. Still open: live provider round trip and live revocation (no provider-contact authority). |
| RISK-PLUGIN-TRUST-EVIDENCE-ABSENT | Plugin isolation and manifests do not yet prove signed vendor roots, revocation, quarantine, rollback, or SBOM/checksum enforcement. | R10 | Medium | Critical | Mitigating — reduced | **SP-025 (2026-08-27/28):** added a 7-test plugin supply-chain adversarial matrix (compromised helper digest, tampered installed artifact, tampered update bundle, untrusted vendor root, tampered retained artifact blocking rollback, quarantine revoking grants, unapproved source/unknown vendor never install) under `EV-SP-025-20260827-PLUGIN-TRUST-INCIDENT-ADR044-01`, plus `docs/operations/PLUGIN_SUPPLY_CHAIN.md`, `INDEPENDENT_SECURITY_REVIEW.md`, and `INDEPENDENT_SECURITY_REVIEW_FINDINGS.md`. Signed vendor roots, hashes, revocation, quarantine, rollback, unverified-code rejection, and helper digest pinning are now enforced and adversarially proven at the deterministic boundary, and a full eight-area independent review found no Critical/High unresolved finding. Still open: public marketplace/vendor PKI and a signed/notarized update transport (owned by R11/ADR-046), and live OS-confinement of a real signed helper/third-party payload. Evidence: `EV-SP-025-20260827-PLUGIN-TRUST-INCIDENT-ADR044-01`. |
| RISK-NOT-NOTARIZED | Development signing may be mistaken for distributable release readiness. | R11 | High | High | Accepted (permanent local-only scope, ADR-049) | Developer ID, hardened nested signing, notarization, staple, clean Gatekeeper evidence. **SP-027 (2026-08-28):** the nested-signing procedure (helpers → extension → app) was validated with the local identity + hardened runtime under `EV-SP-027-20260828-SIGNING-PROCEDURE-02`; `codesign --verify --deep --strict` → Signature OK. **SP-027 local-only scope decision (`EV-SP-027-20260828-LOCAL-ONLY-SCOPE-03`):** external distribution (Developer ID, notarization, external clean-machine) is out of scope. **ADR-049 (2026-08-30, permanent):** the release owner will never acquire or use Developer ID/notarization; AURA is permanently local-only. The signed bundle is local-identity + hardened-runtime only and is NOT presented as external distribution; `GATE-DEVELOPER-ID-NOTARIZATION` is waived. `EV-SP-030-20260830-LOCAL-DEPLOY-01` adds local launch evidence. External distribution would require a new ADR. |
| RISK-NO-SIGNED-UPDATER | No implemented authenticated update/rollback path. | R11 | High | Critical | Mitigating — reduced | Signed manifest/package, atomic update, downgrade/replay protection, rollback tests. **SP-028 (2026-08-29):** implemented deterministic update manifest/package validator, atomic stager with backup/previous-version retention, rollback controller, kill switch, low-disk/interrupted/corruption adversarial tests in `AuraLifecycle`. The default production manifest source returns `.noUpdateAvailable` and no real network/download occurred. ADR-046 remains Proposed pending direct operational evidence of an external signed update. Evidence: `EV-SP-028-20260829-LIFECYCLE-IMPLEMENTATION-01`. |
| RISK-NO-LAUNCH-AT-LOGIN | Continuously available assistant lacks supported user-controlled launch at login. | R11 | High | Medium | Mitigating — reduced | ServiceManagement implementation, UI control, sleep/wake/crash recovery evidence. **SP-028 (2026-08-29):** implemented `LaunchAtLoginController` with protocolized `SMAppService` wrapper, user-intent persistence, enable/disable/status, and health reporting; tests use mock service. Added `LifecycleObserver` launch/crash/sleep/wake state machine with store-backed heartbeat and clean-shutdown flag. **SP-030 (2026-08-31, `EV-SP-030-20260831-R11-POLICY-BLOCK-01`): the previous wording — "live ServiceManagement mutation remains blocked by authority" — was INCOMPLETE and is corrected here.** Authority is not the only blocker, and granting it would not have sufficed. A launch-at-login toggle added to Settings on 2026-08-30 was clicked live on the signed app and failed with `"No matching grant and tier mutation is denied by default"`: `lifecycleLaunchAtLogin` is `.mutation`, `denyByDefaultTiers` covers every tier except `.observation`, and no grant exists because the capability registry disables the whole lifecycle family with the reason "reachable through direct AuraKernel RuntimeAPI calls" — **a compensating control that does not work**. `SMAppService` is never reached. A second, latent layer blocks the obvious repair: `evaluateDirectCapability` throws on `.confirm` although `confirmationPresenter` exists and is wired to four other subsystems. **9 of 11 lifecycle capabilities are denied** (the 2 that work are `.observation` tier), so safe mode, reset, rollback, uninstall, factory reset and the update path are equally unreachable — no SP-028 lifecycle capability has ever been exercised through the product, only by direct test calls. The full suite passes with the defect present, so any fix must add a reachability test and not merely a grant. NO FIX APPLIED: defining a grant is a permission mutation and `authority.mutate_permissions` is `false`, so the decision is the owner's and the live R11 gates cannot run until it is taken. Evidence: `EV-SP-028-20260829-LIFECYCLE-IMPLEMENTATION-01`, `EV-SP-030-20260831-R11-POLICY-BLOCK-01`. **REMEDIATED 2026-08-31 under owner Option A (`EV-SP-030-20260831-R11-POLICY-FIX-01`):** a seeded `lifecycleLaunchAtLogin` grant (`.forRiskTier(.mutation)`) plus the confirmation presenter wired into `evaluateDirectCapability`, and the registry's false reachability reason corrected. Scope is deliberately ONE capability — the eight destructive/network lifecycle capabilities stay deny-by-default, pinned by a new test. The reachability test whose absence let this survive 1317 tests now exists. Suite 1320 tests, 0 failures. **STILL `deterministic_harness` ONLY: no live verification, and `/Applications/AURA.app` still holds the pre-fix binary, so the recorded live denial remains the installed app's behaviour.** Risk stays Mitigating — reduced, NOT closed, until a live run with the owner present shows the confirmation card, the accepted grant, and the login item registered with the OS. **LIVE RUN 2026-08-31 (`EV-SP-030-20260831-R11-LIVE-GATE-01`): PARTIAL.** Rebuilt, re-signed with the same identity (TCC preserved) and installed. The pre-fix denial is **GONE** and the confirmation card is **presented live** (`ONAY GEREKLİ` / `lifecycle.launchAtLogin` / `Risk: 2`), proving both policy layers open — observed, not inferred. **But accepting still does not enable the toggle and no login item is registered, so launch-at-login STILL DOES NOT WORK.** Two further findings: a silent-failure defect where the failure path's error message was erased by its own re-read (**fixed**), and an open design consequence — the *read* `isLaunchAtLoginEnabled()` is gated by the same `.mutation` capability, so Settings-open, toggle and failure each raise a separate 60 s confirmation (**not fixed; changes authorization posture, owner's call**). **SECOND LIVE RUN 2026-08-31 (`EV-SP-030-20260831-R11-LIVE-GATE-02`), owner chose the observation-tier read capability:** opening Settings no longer prompts (verified live) and the failure reason now reaches the user instead of being erased (verified live). That made the real blocker visible: `Permission denied: lifecycle.launchAtLogin was not confirmed` proves the write fires, matches the grant, reaches `.confirm` and invokes the presenter — but **the confirmation card renders in the main AURA panel, not the Settings window that initiated it, and every AURA window dismisses on focus loss, so the user is asked to confirm in a window they are not looking at** and the challenge expires on its 60 s timer. A product defect no unit test would find, present with 1322 tests passing. Deliberately NOT fixed: where a confirmation should be presented is a UI-architecture decision with security consequences, and belongs to the owner. **REMEDIATED 2026-08-31 (`EV-SP-030-20260831-R11-LIVE-GATE-03`), owner authorized the agent to decide:** the confirmation card now renders as the **first row of the Settings form**, so it lives in the window that requested the action. A `.sheet` was tried first and rejected on evidence — inside SwiftUI's `Settings` scene it did not present at all (`sheets=0`, no card in the AX tree) and would have reintroduced the same invisible-confirmation bug in a new form. Closing Settings with a confirmation pending now fails closed. 3 tests; suite 1325, 0 failures. **LIVE VERIFICATION NOT PERFORMED — the display powered off after the install and cannot be woken programmatically.** **FOURTH LIVE ATTEMPT 2026-08-31 (`EV-SP-030-20260831-R11-LIVE-GATE-04`):** `sfltool dumpbtm` shows AURA REGISTERED in Background Task Management (`disabled, allowed, notified`) — proof `service.register()` DID execute successfully at least once, corroborating that the earlier accepted confirmation (LIVE-GATE-01) likely wrote successfully and only the readback failed at the time (a bug since fixed). Four hardened automated click-through attempts did not converge on a clean reproducible run — judged as AX-tree/tooling flakiness in the SwiftUI Settings window, not a product defect (the same script run twice returned different results, unlike every prior defect in this track which was position-independent and reproducible). Risk stays **Mitigating — reduced, NOT closed.** A single manual click by the owner would resolve the remaining uncertainty; `SMAppService` reachability is now proven at the OS level, but full click-through is not automation-verified and R11's live gate remains open. |
| RISK-NO-REPRODUCIBLE-ARCHIVE | A local deterministic ZIP, provenance-bound manifest, bundle inventory, SBOM, and checksum validator now exist, but the toolchain is not fully pinned, CI execution is unobserved, and nested bundle/entitlement/signature/release verification remains incomplete. | R11 | High | High | Mitigating — reduced | Add full-Xcode/pinned-toolchain archive evidence, observed CI artifact retention, bundle-layout/entitlement/signature verification, and external clean-machine checks; development-only evidence must not be treated as release proof. **SP-026 (2026-08-28):** pinned toolchain confirmed and a reproducible `development_unverified` artifact+manifest built at canonical commit `3e81582` (artifact SHA-256 `202bb5cd07386e119fc360a0469acf72e7f1c3347b5d613506b326180a07a1bc`) with a provenance fix + regression test (`EV-SP-026-20260828-REPRODUCIBLE-ARTIFACT-BLOCKED-01`). Deterministic-archive reproduction holds given identical commit+build root; a different build root changes only the 5 compiled Mach-O executables that embed the absolute SwiftPM path. **Observed CI (2026-08-28):** run `33157842324` on `348bb6a` succeeded; development artifact `9680431386` retained with provenance matching the canonical commit (`EV-SP-026-20260828-OBSERVED-CI-COMPLETED-01`). |
| RISK-SP026-CI-OBSERVATION-BLOCKED | The SP-026 observed-CI evidence is blocked because `.github/workflows/ci.yml` requires a self-hosted `macOS, swift-6.4` runner and the runner inventory is empty; SP-026 has no install/configure runner authority. Pushed runs `33152188166`/`33152568023` remain queued with zero completed steps. | R11 / SP-026 | High | High | Closed | Obtain an available self-hosted macOS/swift-6.4 runner under authority, run the workflow, and inspect retained artifacts/signatures/manifests/provenance; distinguish workflow configuration from run evidence. Evidence: `EV-SP-026-20260828-OBSERVED-CI-COMPLETED-01`. |
| RISK-NO-RECOVERY-DIAGNOSTICS | Safe mode, launch-at-login lifecycle, support bundle review, uninstall/factory reset, and update migration recovery are design-only or absent. | R11 | High | Critical | Mitigating — reduced | Implement user-controlled operations with redaction, backups, atomic migration, rollback, low-disk/corrupt-artifact handling, and clean-profile acceptance. **SP-028 (2026-08-29):** implemented safe mode, reset/uninstall/factory reset dry-run semantics, support bundle redaction, migration preflight, config/database migration tables, recovery checkpoints, and lifecycle observation with deterministic tests. Actual reset/uninstall/factory-reset execution on user data and clean-machine recovery are outside current authority. Evidence: `EV-SP-028-20260829-LIFECYCLE-IMPLEMENTATION-01`. |
| RISK-SP022-LIVE-GATE-OPEN | SP-022 deterministic Task Center slice + live UI + live typed-input fail-closed + live durable-task pause/resume on a real claude turn are all evidenced, but a real TCC denial/revocation/restart recovery (a genuine System Settings permission change) was not exercised; the disabled-reason and truthful-state requirements are covered by deterministic + live UI evidence. `taskDelete` stays deny-by-default (no destructive grant). | R9 | Medium | Critical | Mitigating — reduced | SP-022 completed for bounded OPEN-10 scope. A real System-Settings permission-change recovery can be captured under a later user-present session if R9's manual matrix requires it; SP-023 is next. Evidence: `EV-SP-022-20260826-TASK-CONTROLS-SOURCE-01`, `EV-SP-022-20260826-LIVE-UI-01`, `EV-SP-022-20260826-LIVE-DIALOGUE-02`, `EV-SP-022-20260826-LIVE-TASK-CONTROLS-04`. |
| RISK-NO-INDEPENDENT-BETA-EVIDENCE | Unit/integration evidence may not reflect real daily use. | R12 | High | Critical | Mitigating — reduced | SP-029 (2026-08-29) defined an internal local-machine-only closed beta contract under `EV-SP-029-20260829-BETA-CONTRACT-BLOCKED-01` and left the fail-closed readiness record blocked. No cohort enrolled, no telemetry activated, no consent collected, no SLO measured. Still open: actual beta window execution, SLO dashboard, incident review, and independent sign-offs; the contract is a precondition, not evidence of use. |
| RISK-NO-BETA-CONSENT-BOUNDARY | No approved beta cohort, supported-matrix boundary, participant privacy notice, content-free telemetry consent, issue SLA, or rollback/kill-switch owner exists. | R12 | High | Critical | Mitigating — reduced | SP-029 (2026-08-29) created `EV-SP-029-20260829-BETA-CONTRACT-BLOCKED-01` defining the cohort, supported profiles, privacy notice, explicit opt-in, consent withdrawal, retention/access/deletion rights, content-free aggregate telemetry schema (event-class counts, latency histograms, error-code tallies — no transcript/audio/screenshot/content), kill switch, telemetry-off mode, rollback, and incident containment. **SP-029 completion (2026-08-30):** the in-scope content-free aggregate telemetry engine was implemented under `EV-SP-029-20260830-TELEMETRY-AGGREGATOR-01` — opt-in key defaults off and is user-scoped/reversible, recording is a fail-closed no-op unless consent is on, `disableAndPurge()` enforces telemetry-off/consent withdrawal, retention is bounded, and there is **no transport** (nothing leaves the machine). **Owner approval (2026-08-30):** the release owner explicitly approved the contract under `EV-SP-029-20260830-OWNER-APPROVAL-01` and confirmed "ONLARI DA ONAYLIYORUM YAP ARTIK", satisfying the authority component; SP-029 is completed (`EV-SP-029-20260830-CLOSEOUT-01`). Still open: `beta-readiness.json` must remain `blocked` until SP-030 (participant enrollment/consent exercise, live scenario/SLO measurements, independent sign-offs) and SP-031 (ADR-047, signed RC) produce real evidence. |
| RISK-DNS-IP-PINNING-NOT-ENFORCED | `ResolvedIPValidator` implements a correct fail-closed DNS/IP allowlist and is unit-tested, but has **zero production callers**; SP-024 evidence and the Round 1 independent review both describe network enforcement as covering DNS/IP. The documented DNS-rebinding protection is not active on any request. | R10/R12 | Medium | Medium | Accepted (2026-08-30) | Independent review finding F-002 (`EV-SP-030-20260830-SECURITY-REVIEW-01`). Either wire the validator into the production network clients behind an explicit allowlist policy (localhost-only for Ollama; pinned ranges for providers), or record an explicit accepted-risk decision. Any future `security` sign-off must close or explicitly accept this. Note F-004: normalization is textual, so parse to a numeric address before comparing if this is wired. **Owner decision 2026-08-30:** explicitly **accepted as a known gap**, not closed. Rationale: wiring the validator requires inventing an allowlist policy that does not exist; the two production network clients are a loopback Ollama backend and external providers whose addresses are not meaningfully pinnable, and a wrong policy would break legitimate traffic or create false confidence. Compensating controls that ARE active: `URLSessionFactory` (ephemeral session, cookies and cache disabled, every redirect refused) on both callers, and `allowCloudModels = false` in production. This acceptance is **reversible** — if AURA ever leaves local-only scope (ADR-049) or gains a pinnable backend, F-002 must be re-opened and the validator wired (fixing F-004's textual normalization first). Any `security` sign-off must cite this acceptance explicitly. |
| RISK-LIVE-LIFECYCLE-UNVERIFIED | Sleep/wake/crash recovery, safe-mode export, and migration against a populated profile are implemented and unit-tested only — never exercised live on a real Mac. **UPDATE 2026-09-01 (`EV-SP-030-20260901-R11-LIVE-GATE-05`): launch-at-login is now CLOSED.** Root cause found live via AX-tree capture: the confirmation card rendered correctly but above the user's scroll position (the toggle sits below the fold), so it expired unseen — not a state race, as two earlier fix attempts had assumed. Fixed with a ScrollViewReader auto-scroll. Live-verified: toggle accepted, "preference and service updated", `sfltool dumpbtm` shows `enabled` (was `disabled`). Sleep/wake/crash recovery, safe-mode export, and migration remain unit-tested only, never live — this residual is why the row below stays open. **UPDATE 2026-09-03 (`EV-SP-032-20260903-R11-SYNTHETIC-LIFECYCLE-01` + `EV-SP-032-20260903-OWNER-ACCEPTED-GAPS-01`):** the owner directed synthetic closure of the R11 lifecycle gates and accepted the real-host sub-gates as known gaps (option A). The synthetic harness (`SP032LifecycleHarnessTests.swift`) now drives the real production controllers end-to-end (crash recovery, sleep/wake, safe mode, migration preflight, support-bundle export, update stage/rollback + recovery checkpoints, reset/uninstall on throwaway temp dirs); lifecycle suite 60/11/0, full suite 22/0/1337. | R11/R12 | Medium | Medium | Accepted (2026-09-01; re-affirmed 2026-09-03) | `OWNER_SIGNOFF_FALSIFICATION_PACKET.md` Section A "known gap", re-verified and corrected before the `release_recovery` sign-off (`EV-SP-030-20260901-OWNER-SIGNOFF-RELEASE-RECOVERY-01`). **Owner decision 2026-09-01:** explicitly **accepted as a known gap**, not closed, while signing `release_recovery` `obtained`. Compensating evidence for launch-at-login specifically: `sfltool dumpbtm` proves `SMAppService.register()` executed successfully on this Mac at least once (`EV-SP-030-20260831-R11-LIVE-GATE-04`), and the confirmation-card/policy pipeline is proven live-reachable (`EV-SP-030-20260831-R11-LIVE-GATE-01`–`-03`) — what remains is a single human click-through, not an architectural gap. **Owner decision 2026-09-03 (`EV-SP-032-20260903-OWNER-ACCEPTED-GAPS-01`):** accepted the real-host sub-gates (physical sleep/wake, real signed update transport, real clean-profile migration, destructive user-data removal) as known gaps under the synthetic-closure direction, consistent with ADR-049 and the existing accepted-risk rows. This acceptance is **reversible**: re-check before any future `release_recovery` re-review, before any change to R11's local-only scope (ADR-049), and before any future SP-031/SP-032 re-authorization — a single owner-present session exercising these paths would close it outright. |
| RISK-PEER-IDENTITY-PID-BASED | `SecCodeHelperIPCPeerVerifier` resolves a helper's identity via `SecCodeCopyGuestWithAttributes(kSecGuestAttributePid:)`, not an audit token, so it is subject in principle to PID reuse and a check-to-use race. | R10/R12 | Low | Low | Accepted (2026-09-01) | Independent review finding F-003 (`EV-SP-030-20260830-SECURITY-REVIEW-01`). **Owner decision 2026-09-01:** explicitly **accepted as a known gap**, not closed, after investigating whether a real fix is tractable. It is not, safely: `HelperIPCClient.swift` launches the helper over a plain POSIX `Pipe()` (stdin/stdout of a spawned `Process`), not a Unix domain socket and not XPC. Anonymous pipes carry **no OS-level peer-credential mechanism** — no `SO_PEERCRED`/`getpeereid()` equivalent exists for them — so there is no audit token available to switch to without replacing the entire IPC transport with a socket or XPC connection, a structural rewrite of privileged-process communication that is out of proportion to a Low-severity, narrow-exposure finding, and that risks introducing a worse defect than the one it would close. Compensating factors already in place, from the original finding: the client spawns the helper itself and verifies immediately, so the race window is small, and exploitation already requires local code execution — an attacker in that position has a materially larger attack surface than this race offers. This acceptance is **reversible** — if the IPC transport is ever redesigned around sockets or XPC for other reasons, peer identity should move to audit-token verification at that point rather than staying PID-based by default. Any `security` sign-off must cite this acceptance explicitly. |
| RISK-IP-NORMALIZATION-TEXTUAL | `ResolvedIPValidator.normalize` does exact string matching after lowercasing/trimming, so equivalent textual forms of the same address (`::1` vs `0:0:0:0:0:0:0:1`, `::ffff:127.0.0.1` vs `127.0.0.1`) compare unequal. Every mismatch denies, so this is not a bypass on its own. | R10/R12 | Low | Low | Accepted (2026-09-01) | Independent review finding F-004 (`EV-SP-030-20260830-SECURITY-REVIEW-01`). **Owner decision 2026-09-01:** explicitly **accepted as a known gap**, not closed. This finding is inert today: it describes a property of `ResolvedIPValidator`'s comparison logic, and that validator has **zero production callers** per `RISK-DNS-IP-PINNING-NOT-ENFORCED` (F-002, accepted 2026-08-30) — a mismatch in dead code cannot break or bypass anything live. Fixing it now would mean writing a numeric-address parser with no caller to exercise it in production, which is speculative work against an accepted-as-unwired gap rather than a real defect. This acceptance is **explicitly tied to F-002's**: if F-002 is ever re-opened and the validator wired into a live call path, F-004 **must be fixed first** — that ordering is already stated in F-002's own row and is repeated here as its own tracked item. Any `security` sign-off must cite this acceptance explicitly. |
| RISK-TURKISH-LOCALIZATION-COVERAGE | **CORRECTED 2026-08-30 — the original figures were overstated ~3x and are withdrawn (`EV-SP-030-20260830-A11Y-CORRECTION-01`).** Accurate, multi-line-aware count: **13 of 41** accessibility strings in `Sources/AURA` bypassed the language mapping, **now reduced to 8 of 41** (`EV-SP-030-20260830-A11Y-COVERAGE-01`) (originally reported as 38 of 42). The user-facing visible-literal figure (originally 45 of 49) is **withdrawn as unmeasured** — the corrected extractor produced corrupt output and no reliable number was obtained. Several of the 13 interpolate already-localized content, so the substantive static gaps number roughly eight. The **emergency control** (`Emergency Stop`, `Re-arm generated input`, and both VoiceOver hints) is entirely English, so a Turkish-speaking VoiceOver user is read English for the control that stops generated input. | R9/R12 | Medium | Medium | Open | Independent review finding F-005 (`EV-SP-030-20260830-A11Y-REVIEW-01`). Route every user-facing and accessibility string through the existing `language == .turkish` mapping, starting with the emergency control and the VoiceOver surface, and add a test that fails when an accessibility string has no Turkish counterpart. **Partially remediated 2026-08-30 (`EV-SP-030-20260830-A11Y-REMEDIATION-01`):** the safety-critical instance is closed — the emergency control now routes all five of its strings (including both VoiceOver hints) through the `AuraCopy` table, with three regression tests pinning that they resolve, differ between languages, and never revert to the shipped English hints. F-006 is closed: all six `.font(.system(size:))` sites replaced with semantic text styles. **The systemic gap remains Open** — the great majority of user-facing literals and accessibility strings across `Sources/AURA` still have no language conditional, and no repo-wide guard exists (one would currently fail). **`accessibility_localization` stays REFUSED** until coverage is completed or the owner explicitly accepts the residual scope. **Continued 2026-08-30 (`EV-SP-030-20260830-A11Y-COVERAGE-02`):** the deferred plumbing turned out not to be a refactor at all — `AuraConfirmationCard` and `MemoryCorrectionSheet` already held `model`, so only the two-line `language`/`copy()` helper was missing. Wiring them surfaced a **new instance of the same finding**: the entire confirmation card — title, risk/expiry line, and both the Deny and Allow Once buttons — was hardcoded English, so a Turkish user read English at the moment of authorizing a real, side-effecting action. That is the same severity class as the emergency control and it is now closed, along with the memory correction sheet, the message-bubble trace prefix and degraded note, and the menu-bar status label. **The systemic gap remains Open, and one NEW instance was found:** the permission readout (`permissionIndicator`, `AuraMenuView_Tabs.swift:524`) is fed seven hardcoded permission names and `PermissionState.title` (`PermissionCoordinator.swift:14`), which has no language parameter — so both the visible text and the VoiceOver label on the surface that tells a user whether AURA can hear them are entirely English. It was initially misclassified in this same pass as an interpolation-only site and corrected before publication. `MemoryRowView` metadata and the whole of `AuraSettingsView` are also still English. All three were left deliberately because each needs new Turkish that no native speaker has reviewed — **and were then closed 2026-08-30 after the owner was asked and chose "fix it and the rest" (`EV-SP-030-20260830-A11Y-COVERAGE-03`). `AuraCopy` went 72 → 184 keys; the repo-wide guard this register asked for now EXISTS AND PASSES (`AuraCopyTableGuardTests`). F-005's coverage dimension is CLOSED and guarded against regression.** The risk is **not** closed: the 96 new Turkish strings are unreviewed — this session grew the owner's review from ~20 strings to well over a hundred — nothing was verified in a live VoiceOver session, and ADR-050 §4 requires a different reviewer. `accessibility_localization` stays REFUSED **on review independence and unreviewed translation, no longer on coverage**. Residual note; no repo-wide guard exists and one would still fail; and nothing here was verified in a live VoiceOver session (`deterministic_harness` only). |
| RISK-NO-RC-EVIDENCE-PACKAGE | No signed/notarized release-candidate artifact, percentile SLO report, incident remediation record, or independent security/privacy/accessibility/release sign-off exists. | R12 | High | Critical | Open | Keep R12 in_progress; produce a provenance-bound RC evidence package only after R11 and the controlled beta gates pass. SP-029 does not claim RC readiness. **SP-030 attempt (2026-08-30, `EV-SP-030-20260830-PROGRAM-BLOCKED-01`):** SP-030 cannot honestly complete its gate (mandatory SLOs/scenarios/incidents/independent sign-offs require an enrolled+consented cohort, an enabled content-free measurement/transport path, and an independent evaluator — none exists or may be fabricated). `beta-readiness.json` stays `blocked`; R11 dependency remains `in_progress`. SP-031 must NOT start. Completion requires owner-authorized R11 completion + a real consented beta window + independent evaluation. **SP-030 contract pass (2026-08-30, `EV-SP-030-20260830-CONTRACT-MEASURED-MODE-01`, `EV-SP-030-20260830-HARNESS-MEASUREMENT-01`):** the readiness contract previously could not represent a completed beta at all (any recorded measurement was rejected as fabrication), making the gate unreachable by construction; it now supports a provenance-bound measured mode in which sign-offs cannot be self-granted. Two of five SLOs (`false_success` 0.0, `unauthorized_action` 0) and the scenario matrix are recorded as `deterministic_harness` class. Risk stays **Open**: live latency SLOs, live STT/WER, a live-window scenario run, the incident review, and all five independent sign-offs remain outstanding. **SP-030 (2026-08-31, `EV-SP-030-20260831-SLO-INSTRUMENTATION-01`):** `ptt_ack` and `stt_partial` now have a readable source for the first time — two new latency kinds, `PerformanceSampler.percentileSummaries()` (p50/p95/p99, replacing a median/worst-case readout that could not satisfy the contract), and an `STTPipeline` emission of a first-partial latency it had been measuring privately all along. **Both SLOs remain `not_measured` with ZERO samples — instrumentation is not measurement, and `beta-readiness.json` is deliberately untouched.** A contamination defect in that instrumentation was found and fixed before any sample was taken: the permission-prompt grant path recorded the user's reaction time to a modal dialog as machine latency, which would have poisoned the very first `ptt_ack` sample. `stt_partial` cannot be automated — it needs the owner to speak. |
| RISK-FINAL-ACCEPTANCE-BLOCKED | FINAL acceptance cannot pass because R12 is not release_candidate_verified and R11/R12 evidence, security, privacy, accessibility, recovery, and clean-install gates remain open. | FINAL | High | Critical | Open | Keep FINAL blocked/in_progress; return ownership to R11/R12, preserve all exclusions, and require direct evidence or authorized scope decisions before any closure. SP-029 is blocked, not completed; SP-030 must not start. |
