...
| RISK-SP026-CI-OBSERVATION-BLOCKED | The SP-026 observed-CI evidence is blocked because `.github/workflows/ci.yml` requires a self-hosted `macOS, swift-6.4` runner and the runner inventory is empty; SP-026 has no install/configure runner authority. Pushed runs `33152188166`/`33152568023` remain queued with zero completed steps. | R11 / SP-026 | High | High | Blocked | Obtain an available self-hosted macOS/swift-6.4 runner under authority, run the workflow, and inspect retained artifacts/signatures/manifests/provenance; distinguish workflow configuration from run evidence. Evidence: `EV-SP-026-20260828-REPRODUCIBLE-ARTIFACT-BLOCKED-01`. |
