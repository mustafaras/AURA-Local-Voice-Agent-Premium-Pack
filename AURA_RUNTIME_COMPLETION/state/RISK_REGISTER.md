...
| RISK-NO-LAUNCH-AT-LOGIN | Continuously available assistant lacks supported user-controlled launch at login. | R11 | High | Medium | Mitigating — reduced | ServiceManagement implementation, UI control, sleep/wake/crash recovery evidence. **SP-028 (2026-08-29):** implemented `LaunchAtLoginController` with protocolized `SMAppService` wrapper, user-intent persistence, enable/disable/status, and health reporting; tests use mock service. Added `LifecycleObserver` launch/crash/sleep/wake state machine with store-backed heartbeat and clean-shutdown flag. Live ServiceManagement mutation remains blocked by authority. Evidence: `EV-SP-028-20260829-LIFECYCLE-IMPLEMENTATION-01`. |
...
| RISK-NO-RC-EVIDENCE-PACKAGE | No signed/notarized release-candidate artifact, percentile SLO report, incident remediation record, or independent security/privacy/accessibility/release sign-off exists. | R12 | High | Critical | Open | Keep R12 in_progress; produce a provenance-bound RC evidence package only after R11 and the controlled beta gates pass. SP-029 does not claim RC readiness. |
