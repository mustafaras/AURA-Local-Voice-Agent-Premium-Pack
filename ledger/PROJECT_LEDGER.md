...
## 2026-09-02 — SP-030 Computer Use Recovery diagnostic
...
- **Objective / assumptions / risks:** Use only existing local test contracts and the opt-in synthetic Speech route to narrow gaps without the owner present. Preserve the fail-closed distinction between deterministic, synthetic, and live evidence; do not mutate TCC, capture microphone audio, enable telemetry, contact providers, sign, release, deploy, or alter product source.
...
- **Evidence / procedure:** `EV-SP-032-20260903-R11-SYNTHETIC-LIFECYCLE-01`;
  owner directed synthetic closure of R11 lifecycle gates without a user-present
  session. Added `Tests/AuraLifecycleTests/SP032LifecycleHarnessTests.swift`
  (`.serialized`) driving the real production controllers end-to-end with
  synthetic inputs (crash recovery, sleep/wake, safe mode, migration preflight,
  support-bundle export, update stage/rollback + recovery checkpoints,
  reset/uninstall on throwaway temp dirs). Updated the SP-032 prompt.
- **Verdict / class:** `deterministic_harness`. Lifecycle suite 60/11/0; full
  suite 22 bundles/0 failed/1337 tests. Materially advances the R11 lifecycle
  gate; does not close (and must not fabricate) real-host sub-gates (physical
  sleep/wake, real signed update, real clean-profile migration, destructive
  user-data removal). `beta-readiness.json` / `release_candidate` remain
  blocked.
...
### 2026-09-03 — SP-033 ADR-053 synthetic-accepted completion — chain COMPLETE (local scope)
...
### 2026-09-12T15:10Z — UI-0/UI-1 design adoption pass, light-variant accessibility fix, Ollama routing pin, and G1-4 driver repair

- **Objective:** owner asked for UI-0 and UI-1 to be re-applied and improved as design, for the VS Code `Package.swift` load failure to be genuinely closed, and for the Ollama rate limit to be resolved by pinning `glm-5.3-flash:cloud` with a larger answer budget.
- **Package.swift failure — root cause and fix:** the manifest was never at fault. `.build` was a dangling symlink to `/tmp/aura-build`, which `/tmp` cleanup had removed; SwiftPM cannot create a scratch directory through a dangling symlink, so `swift package show-dependencies` failed and vscode-swift reported the wrapped generic message "Error parsing Package.swift" (the real cause goes only to its output channel). `describe` and `dump-package` both succeeded throughout, which is what isolated it. Repointed `.build` to `~/Library/Developer/AURA/build` — durable across reboot, outside Desktop File Provider scope, and gitignored, so no tracked file changed. Acceptance: release bundle built through the default path exit 0, no `com.apple.FinderInfo` anywhere in the bundle, `codesign --verify --deep --strict` → `valid on disk` + `satisfies its Designated Requirement`, proving the codesign rationale that put the scratch dir in `/tmp` still holds at the new location.
- **UI-0/UI-1 design adoption (SEQ-0027):** UI-0's tokens existed but the product never adopted them — `Palette.textPrimary`/`textSecondary`/`surfaceRaised`/`signal` had **zero** usages while surfaces still painted with system colours. Panels, window base, bubbles, identity mark, tab pills and every remaining literal were migrated to the owned vocabulary. **A real accessibility defect was found and fixed:** `hairline`/`textSecondary`/`textTertiary` shipped single-ink (`Color.white.opacity(…)`, no light partner) and measured **1.01:1** on the light surface — all meta and trace text invisible in Light Appearance — while the contrast gate reported green because it composited black ink the implementation never used. Tokens are now appearance-dynamic (6.17:1 / 4.92:1 light), and the gate resolves the *shipped* tokens through `NSAppearance.performAsCurrentDrawingAppearance` instead of a transcription of them, plus a `neutralTokensAreAppearanceDynamic` guard falsified to prove it fires (re-injecting the bug → 2 issues, exit 1).
- **Ollama routing (ADR-059):** the rate limit was the default path, not an accident. `route` ends with "smallest `sizeBytes`" and `:cloud` entries report ~293 bytes against a local model's 5.3 GB, so with `allowCloudModels` defaulting true every request went to cloud. Added `preferredModel` (default `glm-5.3-flash:cloud`), applied **after** every policy filter so a pin can never widen what policy permits; added `responseTokenBudget` (4096) and `thinkingEffort` ("low"), free-text requests only. Measured basis: reasoning left on spent all 4096 tokens on a `thinking` field AURA never reads and returned an **empty** answer; `think: false` moved English reasoning into the answer; `"low"` produced a complete 10,679-character Turkish answer.
- **G1-4 driver repair (SEQ-0028):** the 2026-09-10 lock-screen blocker is gone; a full live turn was driven. `transcript` was found to address the scroll area **positionally** — the pattern the driver exists to avoid — and had broken against UI-1's rebuilt hierarchy. Added `AuraAccessibilityID.conversationTranscript`, repaired the reader to resolve by identifier with a depth-bounded descent and a label/description fallback (message text lives in an accessibility label, not AXValue), and added a `scroll` command. Transcript read: 1 line → 78 lines / 5,779 characters.
- **Verification:** `./scripts/aura-test.sh` full loop run ×2 back-to-back after the design pass (both exit 0, 22/22, `Failed bundles: 0`) and again after the routing/token change (exit 0, 22/22, 0 failed); AURAIntegrationTests 165 tests / 27 suites; AuraAgentTests 247 tests / 9 suites; bundle rebuilt, stable-signed and strict-verified at each stage; live end-to-end turn in the running app produced a seven-region Turkish answer (thinking 20 s → speaking 56 s → idle 80 s).
- **Unresolved risks:** G1-4's jump-to-latest half is unprovable with this harness (AX scrolling does not trigger SwiftUI's scroll-geometry callback) and G1-9 stays held open behind it. Two product defects are recorded but **not** fixed: the assistant's answer renders twice on the conversation surface (transcript bubble and the `lastOperationMessage` "Plan / Doğrulama" panel, which also shows raw `**markdown**`), and runtime-produced English reason strings ("speech complete", "response plan has spoken response") still reach the Turkish UI unmapped. Nothing is committed or pushed — no go-ahead was given this turn. `beta-readiness.json` and `release_candidate` remain blocked per ADR-049.


### 2026-09-12T18:20Z — Delivery completed — four commits pushed, direct-main merge verified, bundle deployed to /Applications

- **Push/merge:** `539012e` (design adoption + light-variant fix), `55c24e4` (Ollama routing pin + generation knobs), `a7e72d3` (acceptance driver repair), `619ab88` (ADR-059 + ledgers) pushed to `origin/main` as `4b4ba4b..619ab88`. Fresh `git ls-remote` returns `619ab88`, equal to local HEAD; `gh pr list --state open` is empty, so direct-main delivery remains the applicable merge route and no merge commit exists.
- **Pre-commit verification:** no Swift file was modified after the last full green suite (checked by mtime against the run log), so that run's evidence stands — `./scripts/aura-test.sh` exit 0, 22/22 PASSED, `Failed bundles: 0`; AURAIntegrationTests 165 tests / 27 suites; AuraAgentTests 247 tests / 9 suites.
- **Deploy:** fresh release bundle built at `~/Library/Developer/AURA/deploy` (exit 0), signed with the stable **"AURA Stable Local Signing"** identity, `codesign --verify --deep --strict` exit 0. The prior install (Sep 10 build) was **moved**, not copied-then-deleted, to `~/Library/Developer/AURA/rollback/AURA.app-20260912-181509` — a durable location, deliberately not `/tmp`, because the previous turn's rollback copy was recorded as lost-on-reboot. Installed by `ditto`; the installed bundle re-verified **CODESIGN_OK**, `CFBundleIconFile` resource present (1,109,018 bytes), launch smoke PID 40956 alive past 12 s running from `/Applications/AURA.app/Contents/MacOS/AURA`, window present, clean `osascript` quit. Installed binary timestamp Sep 12 18:14:14 (was Sep 10 18:11:37), and a window-framed screenshot confirms the deployed surface is the redesigned one.
- **Unresolved risks:** unchanged from the delivery entry — G1-4's jump-to-latest half is unprovable with this harness and G1-9 stays held behind it; the answer still renders twice on the conversation surface and runtime English reason strings still reach the Turkish UI. No beta, release-candidate, notarization, or external-distribution claim is made (ADR-049 unchanged).


### 2026-09-13T16:34Z — UI-2 live status feedback (G2-1..G2-6): motion tokens, listening pulse, TTS equalizer, emergency badge, confirmation entrance

- **Objective:** owner approved UI-1 -> UI-2 with `"go apply be perfect"` (this repo's established explicit-English-equivalent transition token). Implemented and verified G2-1 through G2-5 of UI-2 (Live Status Feedback), then closed the phase's governance gate (G2-6). Full detail: `ui-improvement-plan/ledger/PHASE_LEDGER.md` SEQ-0033 through SEQ-0039; design rationale: `docs/decisions/ADR-060-ui2-live-status-feedback.md`.
- **G2-1 (status pill motion):** three `.animation(_, value: status)` sites on `AuraStatusPill`, split by the vocabulary's own rule — `Motion.smooth` for colour (dot fill, glass tint), `Motion.standard` + `.transition` for the geometry-like text-block crossfade.
- **G2-2 (listening pulse):** `AuraStatusPill.listeningPulseScale` — a `nonisolated` pure function, 0.9…1.15×, applied via a bare `.scaleEffect` with zero `.animation` on the level stream (transform-only, matching the mic-level honesty contract).
- **G2-3 (TTS-driven speaking state):** `AuraAppModel.isSpeakingResponse`, sourced only from two new `TTSStartedEvent`/`TTSStoppedEvent` bus subscriptions, independent of `applyConversationState` (zero lines touched there). New `AuraEqualizer` (3 static bars, one-shot transition, never a synthetic loop) beside the pill in both chrome locations.
- **G2-4 (emergency badge):** new `AuraEmergencyBadge` (amber + symbol + text) added to the header (previously had **no** emergency indicator at all) and restyled into the menu bar panel; one `emergent`-motion pulse on appearance, static under Reduce Motion. The emergency control itself (`AuraMenuView_Tabs.swift`, F-005-tested) is untouched.
- **G2-5 (confirmation card entrance):** `.transition` + `Motion.emergent` added **only** at the conversation-tab call site. The card's second call site, inside `AuraSettingsView`, was deliberately left untouched — it carries a live-verified, evidence-tracked fail-closed fix (`EV-SP-030-20260831-R11-LIVE-GATE-02`) for a real shipped bug, and that record documents a still-open blocker in the same code path.
- **Scoping discipline:** every gate that touched a file outside UI-2's declared hard-boundary list (`AuraDesign.swift` for the shared components, one `AuraMenuView_Content.swift` call site for G2-5) is individually reconciled and recorded in `PHASE_LEDGER.md`; two background-design-doc aspirations (re-tinting the header identity mark; a menu-bar `AuraLevelMeter`) were explicitly **not** implemented, with the reasoning recorded rather than silently dropped.
- **G2-6 verification:** `./scripts/aura-test.sh` full 22-bundle loop ×2 back-to-back, exit 0 both runs, 0 failed bundles; `AURAIntegrationTests` 167 → 176 tests / 28 suites. a11y IDs: `AuraAccessibilityIdentifiers.swift` has zero diff since the UI-1 closure commit — no new interactive controls were introduced, so no new identifiers were required. Fail-closed/emergency-control regression check: the full suite surfaced far more coverage than anticipated (the three `ConfirmationSheetFailClosedTests` cases, window-close, emergency-stop, `confirmationExpiryDenies`/`confirmationChallengeExpiryEnforced` (60 s expiry), the F-005 localization suite) — all green, all unedited. Live driver leg: a fresh signed acceptance bundle, launched via direct exec; the real emergency shortcut (⌘⇧Esc) was sent via `System Events`, and an AX-tree text dump of the running app confirmed the new header badge ("Acil durdurma etkin") genuinely appears where none existed before; app quit cleanly afterward.
- **A genuine re-verification pass (owner-requested "tam ve kusursuz") ran after G2-1..G2-4** and found no discrepancy — a fresh, from-scratch clean build plus a full line-by-line source re-read against the actual gate text, distinct from re-asserting the prior summary (SEQ-0038).
- **Committed, pushed, and deployed under two separate owner tokens:** `"push commit merge deploy"` covered `60a7a0a`/`f24ebfb`/`a210b86`/`409d07d` (icon fix, UI-1 closure + UI-2 open, G2-1..G2-4, deploy record) and installed a release bundle to `/Applications/AURA.app`, launch-smoked. G2-5 and G2-6's work (this entry) remained uncommitted at the time of writing, pending a fresh go-ahead — per this repo's own rule that an approval covers only work finished at the time it is given.
- **Unresolved risks / recorded residual decision points:** (1) TTS-failure stop-reason routing into the error UI (03 §4.3) was not implemented — that behavior lives in `Conversation_TTS.swift`/`Conversation_State.swift`, outside UI-2's scope, and reconciling it against `onSpeechFinished()`'s unconditional idle transition is a `Conversation`-FSM decision for a future phase. (2) The Orb's `isSpeakingResponse: model.status == .speaking` argument (UI-1/G1-6) was not swapped to the new TTS-event-sourced model property, to avoid regression risk on an already-shipped, already-tested surface. Neither blocks UI-3. G2-7 (final machine-coherence close-out and the owner's UI-2 -> UI-3 approval question) remains open.


### 2026-09-14T03:49:24Z — Delivery turn started — commit, push, direct-main merge verification, and local deploy of UI-2 closure

- **Objective:** deliver the already-verified UI-2 G2-5/G2-7 closure represented by the exact current worktree diff: create a scoped commit, push `main`, verify the direct-main merge route and remote equality, build/sign/install the local `/Applications/AURA.app` bundle, launch-smoke it, and leave the ledgers and worktree coherent.
- **Assumptions:** the seven modified tracked paths plus `docs/decisions/ADR-060-ui2-live-status-feedback.md` are the complete authorized scope; `main` is the established delivery branch; “deploy” means the ADR-049-compatible local stable-signed install only. UI-3 remains out of scope and still requires its own approval.
- **Risks:** a remote advance or hosted CI failure after push; an already-running app during replacement; stable-signing or Launch Services/TCC availability; rollback preservation; and the possibility of mistaking local evidence for hosted CI, notarization, beta/RC, or external-release approval.
- **Acceptance criteria:** relevant local tests and static checks pass; `git diff --check` is clean; the scoped commit is pushed; fresh `git ls-remote` equals `HEAD`; open-PR state is checked; the release bundle is built, stable-signed, strict-verified, installed, re-verified, and launch-smoked with clean quit; final ledgers record exact receipts, unresolved risks, and the remaining UI-3 approval boundary; no beta/RC/notarization/external-release claim is made.

### 2026-09-14T03:56:23Z — Delivery completed — UI-2 closure pushed through direct-main and deployed locally

- **Commit/push/merge:** scoped commit `e5f83e4f0c277d3b62c9e22feff5c84a0f104637` (`feat(ui): close UI-2 live status feedback phase`) pushed from `main` to `origin/main`. Fresh `git ls-remote origin refs/heads/main` returned the same full hash; `git rev-list --left-right --count HEAD...origin/main` returned `0 0`; `gh pr list --state open` returned `[]`. Direct-main is the applicable merge route, with no separate merge commit.
- **Local verification:** `git diff --check`, `zsh -n scripts/*.sh`, strict-concurrency `swift build`, repository-hygiene/supply-chain/second-pass validators, and `./scripts/aura-test.sh /tmp/aura-delivery-tests` passed. The full matrix completed 22/22 bundles with `Failed bundles: 0`; `AURAIntegrationTests` reported 176 tests / 28 suites and `AuraAgentTests` reported 247 tests / 9 suites. The broader Python governance unittest run executed 96 tests with one pre-existing error: the archived runtime-completion schema rejects the existing `repository_commit` property in `archive/runtime-completion/state/program-state.json`. No current-scope file touches that archive; hosted CI is therefore not claimed green.
- **Deploy:** `BUILD_DIR=/Users/m_ras/Library/Developer/AURA/deploy-ui2-20260914 ./scripts/build-app-bundle.sh` completed; `scripts/codesign-adhoc.sh` selected `AURA Stable Local Signing`; `scripts/verify-signature.sh` and `codesign --verify --deep --strict` passed. The signed main executable SHA-256 was `151807cd8761f35ecffe5c4cd9dae94b163aa9264114460d375438cd62668a75`. The previous `/Applications/AURA.app` was moved to the durable rollback path `/Users/m_ras/Library/Developer/AURA/rollback/AURA.app-20260914-065500`; the new bundle was installed, strict-verified, launch-smoked as PID `53227`, and quit cleanly.
- **Remaining boundary:** the installed app is a local stable-signed development deployment only. No Developer ID, notarization, beta/RC, hosted-CI, external publication, or release approval is claimed. UI-2 remains `awaiting-approval` for UI-3; residual TTS-failure routing, the Orb speaking-state source decision, and the intentionally untouched Settings confirmation-card path remain recorded in ADR-060/phase ledgers.

### 2026-09-14T14:26:40Z — UI-3 Information Architecture opened

- Objective: execute the owner-approved UI-3 phase: identifier-preserving sidebar navigation, real task-progress payload projection, bounded in-memory latency sparklines and telemetry, Recovery status rows, and a fail-closed static ⌘K command palette.
- Authority and assumptions: the user supplied the UI-3 prompt and the explicit token "go apply be perfect"; UI-3 is the only active phase. AuraProductTab raw values and existing accessibility identifiers remain compatibility anchors. The prompt's hard-boundary file list governs implementation; no UI-4 work or release action is authorized.
- Risks: sidebar AX hierarchy may regress the AppleScript driver; progress events may race task snapshots; pull-cadence history must not be presented as sample-level truth; palette destructive actions must use the existing confirmation card; chart colors must pass graphical contrast.
- Acceptance criteria: G3-1 through G3-8 pass with source, unit, full-suite, validator, and live driver evidence; changed files remain within the active scope; ADR, CURRENT_STATE, and this append-only ledger are current; no commit, push, deploy, or release is performed.

### 2026-09-14T16:07:00Z — UI-3 Information Architecture complete locally — awaiting approval

- **Implementation:** UI-3 changed only the approved UI source/test surface plus the explicit G3-7 governance artifacts. The panel now uses an identifier-preserving sidebar; real `TaskProgressEvent` payloads drive clamped task rings; Recovery renders bounded in-memory p50/p95/p99 telemetry with budget ticks, breach markers, sparklines, provenance, `AuraStatusRow`, and contrast-tested data-viz colors; the static ⌘K palette dispatches to existing owners and routes Launch-at-Login through the existing fail-closed confirmation path.
- **Gate receipts:** G3-1 through G3-6 are recorded in phase-ledger SEQ-0043 through SEQ-0048, including the live signed temporary-app AppleScript leg across all six tabs, Recovery deck lookup, real ⌘K, palette entries, existing confirmation card, and Escape dismissal. G3-7 is SEQ-0049; G3-8 and the five-question cognitive completion record are SEQ-0050.
- **Verification:** `/tmp/aura-ui3-g37-run1` and fresh `/tmp/aura-ui3-g37-run2b` both exited 0 with 22/22 bundles and `Failed bundles: 0`; `AURAIntegrationTests` was 191 tests / 32 suites in both runs. `swift build`, `git diff --check`, and touched-script `zsh -n` completed without errors. `python3 -m unittest discover -s scripts/tests` ran 96 tests with 95 passes and one archived runtime-completion state mismatch (`working_tree_state=clean` versus the intentionally uncommitted dirty UI-3 worktree); the frozen archive was not edited and hosted-CI green is not claimed.
- **Governance:** `docs/decisions/ADR-061-ui3-information-architecture.md` added; this ledger appended; `ledger/CURRENT_STATE.md` updated with a new top UI-3 state entry; `ui-improvement-plan/ledger/CURRENT_PHASE.md` and `PHASE_LEDGER.md` closed the phase. `bash ui-improvement-plan/validate-continuity.sh` returned `VALIDATOR: OK - machine coherent` with all eight gates passed and `phase_status: awaiting-approval`.
- **Boundary and residual risk:** no commit, push, deploy, release, or `/Applications/AURA.app` replacement was performed. The temporary acceptance bundle was outside `/Applications`; local stable signing and live AX evidence do not establish device deployment, hosted CI, notarization, beta/RC, or external release. Telemetry remains summary-pull cadence, and the existing confirmation card's tab placement was not broadened.
- **Next safe action:** stop at the UI-3 approval boundary. UI-4 has not started and requires a new explicit owner approval.

### 2026-09-14T19:18:00+03:00 — UI-3 delivery approved, pushed, merged on main, and deployed locally

- **Owner authorization:** explicit `push commit merge deploy` received after the UI-3 G3-8 closure. No UI-4 work started.
- **Commit/merge:** `ca753296b4d77f89a5d22dbc263b79735dfa98c1` (`feat(ui): close UI-3 information architecture phase`) committed and pushed from `main` to `origin/main`. Remote hash matched; `HEAD...origin/main` was `0 0`; `gh pr list --state open --base main` returned `[]`. This repository uses direct-main, so no separate merge commit was needed.
- **Bundle and signature:** `BUILD_DIR=/Users/m_ras/Library/Developer/AURA/deploy-ui3-20260914 ./scripts/build-app-bundle.sh` exited 0. `scripts/codesign-adhoc.sh` selected `AURA Stable Local Signing`; `scripts/verify-signature.sh` and `codesign --verify --deep --strict` passed. The deployed main executable SHA-256 is `9f6313da00f716a605702be2b593e2aad573e75301c8adbe3bf7b21da811d8aa`.
- **Install and smoke:** old `/Applications/AURA.app` moved recoverably to `/Users/m_ras/Library/Developer/AURA/rollback/AURA.app-20260914-191651`; the new app was installed at `/Applications/AURA.app`, strict-verified, and hash-matched the build bundle. `open` exit 0, live process PID `29314`, driver `window-already-open`, driver status `Boşta`, clean AppleScript quit, and no remaining AURA process were observed.
- **Boundary:** this is a local stable-signed deployment only. No Developer ID, notarization, beta/RC, hosted-CI, external publication, or release approval is claimed. The rollback copy remains available. The follow-on governance commit records this delivery receipt; the deployed app source is exactly `ca753296…`.

### 2026-09-14T19:29:00+03:00 — UI-3 post-delivery governance recheck
...
### 2026-09-16T15:04:23+03:00 — PA-0 Owner Trust Posture implemented and verified (ADR-064); owner attestation pending

- **Objective:** PA-0 of `personal-assistant-plan/` (opened by `APPROVAL: PLAN -> PA-0`, token `ONAY PA-0`, 2026-09-16; D-1…D-6 recorded, D-2 overridden by the owner to "lift computer-use guards A/B/C, keep emergency stop"). Make the policy engine answer `.allow` with no challenge for the owner on every registered capability, and lift the computer-use structural guards for the owner posture, through one compile-time switch and one ADR, leaving the engine, audit, emergency stop, identity-change detection, `maxIterations`, and the rate limit untouched.
- **Delivered:** `Sources/AuraPolicy/OwnerTrustPosture.swift` (`isEnabled = true`); `DefaultPolicyGrants` derives the eight formerly challenging seeds (`shell.exec`, four agent/cloud runs, `app.terminate`, `lifecycle.launchAtLogin`, `computerUse.run`) from the switch via `grants(ownerTrustEnabled:)`; nine registered-but-unseeded capabilities found by the new coverage scan and seeded `.none`/`.any` (`agent.run`, `app.hide`, `vscode.runTask/cancelTask/runTests/cancelTests`, `computerUse.interact/mutate/destructiveAct` — the last three had **never** been seeded, so computer use could only observe in production under ADR-055); `Sources/AuraComputerUse/ComputerUseGuardPosture.swift` (`.structural` / `.ownerTrust`, `.production` derived) threaded through `ComputerUseControlLoop`, `AXCGEventActionExecutor`, and `ScreenContextEngine(sensitiveApplicationExclusionEnabled:)` with production defaults (kernel untouched); comment-only corrections in `ComputerUseTypes.swift` and `Configuration_ComputerUseConfiguration.swift`; `docs/decisions/ADR-064-owner-trust-posture.md` (Accepted).
- **Tests:** new `OwnerTrustPostureTests` (6), `OwnerGrantCoverageTests` (4, manifests + computer-use step capabilities + screen.capture), `ComputerUseGuardPostureTests` (18, each guard lifted/refused per preset; emergency stop stops both). Pinned updates, each `// ADR-064`: `DefaultPolicyGrantsTests` (6 assertions, 1 repoint, inventory +9), `SP006LiveCapabilityScenarios` opt-in deny-presenter leg (also fixed a vacuous pre-existing assertion that searched `shell.execute_typed` while the router emits `shell.execute`); guard-mechanics fixtures gained only an explicit `.structural`. Full `scripts/aura-test.sh` loop ×3: 22/22 bundles, 0 failed (`personal-assistant-plan/evidence/PA-0/g0-5-full-suite-run1.log`, `g0-7-full-suite-run2.log`, `g0-7-full-suite-run3.log`).
- **Live (PA-0 bundle `~/Library/Developer/AURA/pa0-20260916/AURA.app`, AURA Stable Local Signing, strict OK, main executable SHA-256 `2c5b8f0739423f5222834fd3c0489f3e83f86467b0eb4986b54d2de432aa5dd4`, launched via `open -a`):** store `aura.policy.grants` reconciled 32 → 41 grants, all `.none`, all seed-marked (`reconcileSeededGrants` proven live). Driver turns with `find aura.confirmation.card` → not found on each: `run date` → "Command completed with exit code 0."; `claude say hello` → no card, but the run failed before policy at `CodingTaskCoordinator.preflight` ("no existing workspace candidate") — pre-existing, PA-3 scope; `quit Notes` → "Quit com.apple.Notes.", Notes terminated. A first `quit Notes` attempt raised a card because the **installed old `/Applications/AURA.app` had been launched at 14:43** and rewrote the store with its 32-grant seed — expected pre-PA-0 behaviour, invalidated and retried on the PA-0 process. Cropped screenshots under `personal-assistant-plan/evidence/PA-0/`.
- **Pre-existing findings recorded, not fixed:** SP006 opt-in health-row pins (28/14/14 vs registry 42/29/13, stale since `39d5902`); Ollama live NLU leg returns fenced JSON; login item points at a stale acceptance bundle (`aura-ui4-g44`, PA-2 scope); coding-agent NLU route supplies no workspace (PA-3 scope).
...
### 2026-09-16T18:01:38+03:00 — PA-1 One Identity, One Consent implemented and verified (ADR-065)

- **Objective:** PA-1 of `personal-assistant-plan/` (opened by `APPROVAL: PA-0 -> PA-1`, token `ONAY PA-1`, 2026-09-16): make every bundle executable carry the stable local identity, keep non-stable builds out of the production Keychain namespace, and collect all six macOS permissions once in one guided pass so relaunch and rebuild never re-prompt.
- **Delivered:** G1-0 process→permission map (`personal-assistant-plan/evidence/PA-1/permission-map.md`): every TCC-protected call executes in the main `AURA` process; helpers are packaged but never launched (ADR-034 §7 / ADR-044) → one TCC subject, the automation helper needs no Accessibility grant. G1-1 `scripts/codesign-adhoc.sh` (ad-hoc only with `AURA_ALLOW_ADHOC=1`, else exit 2 with the ADR-030 instruction) and `scripts/verify-signature.sh` (per-object identity invariant: no `Signature=adhoc`, `Authority=AURA Stable Local Signing`, DR pins the keychain certificate's SHA-1); identity inventory with `notAfter` 2036-07-26. G1-2 `Sources/AuraSecurity/CodeIdentityProbe.swift` (Security-framework probe, injectable), `AppConfiguration.effectiveServiceName` (`.dev` suffix for non-stable identities), kernel wiring for the app / Safari-bridge / VS Code-bridge Keychain stores. G1-3 `PermissionKind` + six-field `PermissionSnapshot`, `requestAllPermissions()` single pass in the `privilegedAccess` stage (stage machine untouched), six live rows with System Settings fallbacks, Calendars/Contacts rows on Privacy and Recovery tabs, indicator identifiers, EN/TR copy. ADR-065 Accepted.
- **Tests:** `CodeIdentityProbeTests` (7), `ServiceNameDerivationTests` (4), `PA1ConsentPassTests` (8); UI-5 pinned copy updated (ADR-065 comment); acceptance driver `scanLimit` 150 → 400. Full `scripts/aura-test.sh` loop ×3: 22/22 bundles, 0 failed (`evidence/PA-1/g1-5-full-suite-run1..3.log`).
- **Live:** verifier positive on `/Applications/AURA.app` (6 × stable identity OK) and negative on an ad-hoc copy (exit 1); ad-hoc debug binary launched 20 s → production Keychain item counts unchanged, no `.dev` items, no SecurityAgent activity; persistence protocol on stable-signed bundles launched via `open -a`: fresh path (SHA `875dd6d9…`), relaunch ×3, clean rebuild (SHA `d9763feb…`, designated-requirement diff empty) — six indicators `Verildi` on every launch, prompt-window count 0, TCC unified log withheld (recorded, no FDA requested). Owner attestation verbatim (SEQ-0021/0022): "onaylıyorum: PA-1 açılışlarında hiçbir macOS izin diyaloğu ve hiçbir Keychain parola diyaloğu çıkmadı".
...
### 2026-09-17T10:56:46+03:00 — PA-2 Launch at Login by Default, Always On implemented and verified (ADR-066)

- **Objective:** PA-2 of `personal-assistant-plan/` (opened by `APPROVAL: PA-1 -> PA-2`, token `ONAY PA-2`, 2026-09-16): under the owner posture the installed AURA registers itself as a login item by default, idempotently, without a confirmation card; macOS's `.requiresApproval` answer is shown honestly with a verified deep link; the onboarding `launchAtLogin` stage becomes informational; a real logout → login brings AURA up by itself with no prompt.
- **Delivered:** G2-1 `Sources/AuraLifecycle/LaunchAtLoginController.swift` — `defaultEnabled` (posture as configuration), `userPreferenceEnabled()` treats the schema `false` in `.secureDefaults` as a placeholder and lets the user's `userSettings` value win, `ensureRegisteredAtLaunch()` (register only when preference on and status not enabled; no preference-layer write; `.failed` health instead of throwing; no re-`register()` under `.requiresApproval`); `LaunchAtLoginStatus` raw values corrected to the `SMAppService.Status` contract (notRegistered 0 / enabled 1 / requiresApproval 2 / notFound 3) — previously `notRegistered` decoded as "unsupported" and `requiresApproval` as `notFound`; `LaunchAtLoginService.swift` surfaces `.requiresApproval`; `AuraKernel_StartStop.swift` post-start hook, `AuraKernel_RuntimeAPI.swift` / `AuraKernel_Construction.swift` wiring; Settings › Startup row with the honest notice and the verified `x-apple.systempreferences:com.apple.LoginItems-Settings.extension` anchor (opened live → "Login Items"). G2-2 `ProductUIState.swift` §3.2 statement EN/TR, `AuraMenuView.swift` `launchAtLoginStatusRow` (live on / awaiting approval / off), identifiers in `AuraAccessibilityIdentifiers.swift`; stage machine (ADR-063) untouched — `git diff --stat` on R9 stage tests empty. ADR-066 Accepted. 13 files changed, +264/−18, plus 3 new test files and the ADR.
- **Tests:** `PA2LaunchAtLoginDefaultTests` (10: raw values, in-memory contract, default on/off, explicit off beats default, register once then `changed: false`, off no-op, no session override, requiresApproval honest + no re-register, register landing in requiresApproval, failure recorded); `PA2LaunchAtLoginRowTests` (4); `PA2LaunchAtLoginStageTests` (4: copy EN/TR, stage order/optionality unchanged, identifiers, 2 languages × 3 states). Full `scripts/aura-test.sh` loop ×3: 22/22 bundles, 0 failed, FULL_EXIT=0 each (`personal-assistant-plan/evidence/PA-2/g2-5-full-suite-run1..3.log`; bundle count cross-checked against `ls Tests/` = 22).
- **Live:** G2-3 fresh stable-signed bundle (`pa2-20260916`, SHA `07e93d01…`, verifier 6/6) launched via `open -a` → driver `status` `Boşta` in 4.69 s, no onboarding sheet, dialog windows 0/0/0, Privacy tab 6 × `Verildi`, four read-first integrations resolved (`probeExternalAvailability` ran), health `launch-at-login — enabled=true, status=1, already registered`. Owner deploy to `/Applications` (SEQ-0031); first installed-bundle launch re-pointed `sfltool dumpbtm` from the build path to `/Applications/AURA.app` (carried finding from UI-4 closed); Settings off → BTM `[disabled …] (0xa)`, on → `[enabled …] (0xb)`. G2-4 real logout → login (owner): console session 10:40, AURA PID 732 started by launchd at 10:41:01 from `/Applications/AURA.app`, `Boşta`, dialogs 0/0/0, no sheet, `dumpbtm` `/Applications/AURA.app` enabled; owner attestation verbatim (SEQ-0032): "tamam onaylıyorum".
- **Findings recorded, not worked around:** BTM follows the bundle identifier's last-launched location (so the installed bundle must be the one launched after deploy — it was); on this macOS 27 host `unregister()` leaves a *disabled* BTM record rather than deleting it (the Open-at-Login table drops it; the plan's "no longer lists" is satisfied as "no longer enabled").
