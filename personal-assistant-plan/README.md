...
**Status:** Approved 2026-09-16 (`ONAY PA-0`, D-1…D-6 recorded in `ledger/PHASE_LEDGER.md`); PA-0 in progress.
**Date:** 2026-09-15
...
What this plan **does not** do: it does not remove the policy engine, the audit trail, the emergency stop, the prompt-injection classifier, or the network allowlist. (The computer-use structural guards — secure-field halt, sensitive-app exclusion, no-progress and destructive-action detection — were originally in this list; the owner removed them on 2026-09-16 via D-2, and PA-0 lifts them for the owner posture under ADR-064 while keeping the emergency stop.) `AGENTS.md` says "Never bypass the permission engine for convenience" — this plan *reconfigures* the engine through seeded grants and ADRs (the exact route ADR-055 already used); it does not bypass it. Every call is still evaluated and audited; the *answer* the engine gives the owner changes.
...
| D-2 | Emergency stop and the structural computer-use guards remain | **Resolved 2026-09-16 (owner overrode the recommendation):** the computer-use structural guards are **removed** for the owner posture — (A) the mandatory-confirmation intent gate, (B) secure-field refusal in the loop and the executor plus the screen-context sensitive-application exclusion, (C) the unexpected-modal halt, the no-progress halt, and the per-plan step ceiling. **Emergency stop stays** (owner's explicit exception), as do identity-change detection, the `maxIterations` resource ceiling, and the action rate limit (bounds, not guards). Every guard derives from `OwnerTrustPosture` so the code path survives for the disabled posture and for tests; ADR-064 records the waiver. Original recommendation was *Keep*. |
