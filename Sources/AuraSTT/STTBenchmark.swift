...
public enum STTBenchmark {
    ...
    public static func wordErrorRate(reference: String, hypothesis: String) -> Double {
        let refTokens = reference.lowercased().components(separatedBy: .whitespacesAndNewlines).filter { !$0.isEmpty }
        let hypTokens = hypothesis.lowercased().components(separatedBy: .whitespacesAndNewlines).filter { !$0.isEmpty }
    ...
    public static func entityErrorRate(reference: String, hypothesis: String, entities: Set<String>) -> Double {
