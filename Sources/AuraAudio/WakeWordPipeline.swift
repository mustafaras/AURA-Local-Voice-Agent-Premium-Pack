...
public actor WakeWordPipeline {
    ...
    public func setPrivacyMode(_ enabled: Bool, triggeredByKeyboardShortcut: Bool = false) async {
        privacyMode = enabled
        await emitPrivacyModeEvent(enabled: enabled, triggeredByKeyboardShortcut: triggeredByKeyboardShortcut)
        if enabled {
            state = privacyModeRequiresShortcut && !triggeredByKeyboardShortcut ? .privacyArmed : .listening
        } else {
            state = .listening
        }
