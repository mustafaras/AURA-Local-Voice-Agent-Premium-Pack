...
public actor AuraAudio {
    ...
    public func start() async throws(AuraError) {
        ...
        do {
            ...
            await emitCaptureStarted(deviceID: inputFormat.channelLayout?.description, correlationID: correlationID)
    ...
    public func stop(reason: String = "requested") async {
        ...
        await logger.info("Stopping audio capture: \(reason)", correlationID: correlationID, actor: .audio)
    ...
    private func handleCapturedBuffer(
        ...
        var error: NSError?
        let status = converter.convert(to: outputBuffer, error: &error) { (_: AVAudioPacketCount, outStatus: UnsafeMutablePointer<AVAudioConverterInputStatus>) -> AVAudioBuffer? in
