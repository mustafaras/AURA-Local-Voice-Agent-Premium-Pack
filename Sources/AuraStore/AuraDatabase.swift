...
public actor AuraDatabase {
    ...
    public func query(sql: String, arguments: [SQLiteValue]) throws(AuraError) -> [[String: SQLiteValue]] {
        guard let statement = try prepare(sql: sql) else {
            throw AuraError.storeError("Failed to prepare statement: \(sql)")
        }
        defer { sqlite3_finalize(statement) }
        ...
        for (index, value) in arguments.enumerated() {
            ...
            case .blob(let data):
                result = data.withUnsafeBytes { rawBuffer in
                    sqlite3_bind_blob(statement, position, rawBuffer.baseAddress, Int32(data.count), unsafeBitCast(-1, to: sqlite3_destructor_type.self))
                }
            }
            guard result == SQLITE_OK else {
